...
<%!    public static String formatNumberList = "#,###";
    ...
    public String drawList(int language, Vector objectClass, long roomId, String approot, int start, int recordToGet) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            try {
                roomClass = PstRoomClass.fetchExc(room.getRoomClassId());
            } catch (Exception e) {
                System.out.println("Room class not found ...");
            }
            rowx.add(cekNull(roomClass.getClassName()));
                        ...
                        <tr> 
                            <td height="100%" width="100%" colspan="2"> 
                                <table border="0" cellspacing="1" cellpadding="1" width="100%">
                                    ...
                                    <tr align="left">
                                        <td valign="top" width="12%"><%=textListHeader[SESS_LANGUAGE][5]%></td>
                                        <td width="1%" valign="top">:</td>
                                        <td width="87%" valign="top">
                                            <%

                                                Vector val_roomclass = new Vector(1, 1);
                                                Vector key_roomclass = new Vector(1, 1);
                                                Vector vt_roomclass = PstRoomClass.list(0, 0, "", PstRoomClass.fieldNames[PstRoomClass.FLD_POS_ROOM_CLASS_ID]);

                                                for (int d = 0; d < vt_roomclass.size(); d++) {
                                                    RoomClass roomclass = (RoomClass) vt_roomclass.get(d);
                                                    val_roomclass.add("" + roomclass.getOID() + "");
                                                    key_roomclass.add(roomclass.getClassName());
                                                }


                                            %>
                                            <%=ControlCombo.draw(frmRoom.fieldNames[FrmRoom.FRM_FIELD_ROOM_CLASS_ID], "formElemen", null, "" + room.getRoomClassId(), val_roomclass, key_roomclass, null)%>
                                        </td>
                                    </tr>
                                    <tr align="left">
