...
<%!//    
    //list jewelry
    public String drawListJewelry(int language, Vector objectClass, int start, int iCommand, 
            int showImage, FrmMaterial frmMaterial) {
        String result = "";
        ...
        if (objectClass != null && objectClass.size() > 0) {
            ...
            ctrlist.addHeader(//textListMaterialHeader[language][0]+"</br>"
                    "<a onclick=\"javascript:checkAll();\" href=\"javascript:void();\">check all</a></br>"
                    +"<a onclick=\"javascript:unCheckAll();\" href=\"javascript:void();\">uncheck all</a></br>","3%");
            ...
            ctrlist.addHeader("Faktor","10%"); 
            ctrlist.addHeader("Berat","10%"); 
            ctrlist.addHeader("Etalase","10%");
            ctrlist.addHeader("Remarks","10%"); 
            ...
            for (int i = 0; i < objectClass.size(); i++) {
                ...
                MaterialDetail matDet = new MaterialDetail();
                try{
                    matDet = PstMaterialDetail.fetchExcMaterialDetailId(matUpdate.getOID());
                }catch(Exception exc){
                ...
                String etalase = "";
                try{
                    matKsg = PstKsg.fetchExc(matUpdate.getGondolaCode());
                ...
                rowx.add("<table style='background: #EBF6FF;'><tr><td style='border: none;'><input type=\"checkbox\" name=\"check_all\" value=\""+matUpdate.getOID()+"\" checked></td>"
                        +"<td style='border: none;'><input type=\"text\" name=\"qty_"+i+"\" size=\"1\" value=\"1\" placeholder='qty'></td></tr></table>");//+start);
                rowx.add("<a title=\"\" href=\"javascript:cmdEdit('"+matUpdate.getOID()+"')\">"+matUpdate.getSku()+"</a>"
                        + "<input type=\"hidden\" name=\""+frmMaterial.fieldNames[frmMaterial.FRM_FIELD_MATERIAL_ID]+"_"+i+"\" value=\""+matUpdate.getOID()+"\">");

                rowx.add(""+matUpdate.getFullName());

                rowx.add(""+String.format("%,.2f",matDet.getFaktorJual()));
                rowx.add(""+String.format("%,.3f",matDet.getBerat()));
                rowx.add(""+etalase);
                rowx.add(""+matUpdate.getMaterialDescription());
...
<html><!-- #BeginTemplate "/Templates/main.dwt" --><!-- DW6 -->
    <head>
        ...
        <script language="JavaScript">
            document.getElementsByName('check_all').checked = true;
            function cmdAdd()
            {
            ...
            function cmdSave()
            {
                ...
                document.frmmaterial.action = "list_print_barcode.jsp";
                document.frmmaterial.submit();
            }
            ...
            function cmdBack()
            {
                ...
                document.frmmaterial.action = "src_print_barcode.jsp";
                document.frmmaterial.submit();
            }                
            ...
            function printCheck()
            {
              ...
              for (var i=0; i<checkboxes.length; i++)  {
                if (checkboxes[i].type == 'checkbox' && checkboxes[i].checked == true)   {
                    oid_mat_list += ";"+checkboxes[i].value;
                    qtyAll += ";"+document.getElementsByName("qty_"+i)[0].value;        
                }
              }
              if(oid_mat_list != "" && oid_mat_list != null){
                    window.open("<%=approot%>/PrintBarcodeIssuer?mat_oid="+oid_mat_list+"&qty_all="+qtyAll,"_blank");
              }else{
                                ...
                                <form name="frmmaterial" method="post" action=""> 
                                    ...
                                    <table width="100%" cellspacing="0" cellpadding="3">
                                        <tr align="left" valign="top">
                                            ...
                                            <td height="22" valign="middle" colspan="3">                                                
                                                <%=drawListJewelry(SESS_LANGUAGE, records, start, iCommand, srcMaterial.getShowImage(), frmMaterial)%>
                                            </td>
                                            <%} else {%>
                                    ...
                                    <table width="91%" border="0" cellspacing="0" cellpadding="0">
                                        <tr>
                                            ...
                                            <td nowrap class="command">
                                                <a class="btn btn-primary" href="javascript:cmdBack()"><i class="fa fa-arrow-left"></i> <%=ctrLine.getCommand(SESS_LANGUAGE, dfTitle, ctrLine.CMD_BACK_SEARCH, true)%></a>
                                                <a class="btn btn-danger" href="javascript:printCheck()"><i class="fa fa-print"></i> Print Barcode</a>
                                            </td> 
                                        </tr>                                        
                                    </table>
                                    <br>                                        
