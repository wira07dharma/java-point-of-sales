<%-- 
    Document   : vdrmaterialpricecontract
    Created on : Mar 4, 2014, 3:19:27 PM
    Author     : dimata005
--%>
...
public static final String textListTitleHeader[][] =
{
	{"Daftar Harga Supplier","SKU","Klik sku untuk kembali ke edit barang","Nama",
	 "Tidak ada data vendor price ..","Nama Barang","Kode Contract","Unit Order","Harga Beli","Barcode",
	  "Discount Terakhir","Keterangan","PPN Terakhir","Harga Beli Terakhir",
	  "Tambah Harga Supplier","Harga Supplier","Kembali ke Supplier","Mata Uang","Biaya Kirim","Diskon 1 Terakhir","Diskon 2 Terakhir","%"},
	{"Supplier Price List","SKU","Click sku for back to goods edit","Name",
	 "No data vendor price ..","Supplier","Code","Unit Order","Buying Price","Barcode",
...
public String drawList(int iCommand, Vector objectClass,int start,int SESS_LANGUAGE)

	{
		...
		for (int i = 0; i < objectClass.size(); i++) {
                    ...
                    rowx.add("<a href=\"javascript:cmdEdit('"+String.valueOf(matVendorPrice.getOID())+"','"+String.valueOf(matVendorPrice.getMaterialId())+"')\">"+material.getName()+"</a>");
...
long vendorId = FRMQueryString.requestLong(request, "hidden_contact_id");
...
long vendorContractId = FRMQueryString.requestLong(request, "hidden_vendorContractId");
...
/*switch statement */
iErrCode = ctrlMatVendorPrice.action(iCommand , vendorContractId);
/* end switch*/
FrmMatVendorPrice frmMatVendorPrice = ctrlMatVendorPrice.getForm();
whereClause = ""+PstMatVendorPrice.fieldNames[PstMatVendorPrice.FLD_VENDOR_ID]+"="+vendorId;
...
function cmdEdit(hidden_vendorContractId, materialId){
	document.frmmaterial.hidden_vendorContractId.value=hidden_vendorContractId;
        document.frmmaterial.hidden_material_id.value=materialId;
...
function cmdBackList(){
        document.frmmaterial.hidden_contact_id.value="<%=vendorId%>";
	document.frmmaterial.command.value="<%=Command.BACK%>";
	document.frmmaterial.action="contact_company_edit.jsp";
	document.frmmaterial.submit();
}
...
function calculateAll(){
	...
	function check(){
		//window.open("vendordosearch.jsp?command=<%=Command.FIRST%>&txt_vendorname="+document.frmmaterial.txt_vendorname.value+"&frm_id=0","materialvendor", "height=500,width=800,status=yes,top, scrollbars=yes,toolbar=no,menubar=no,location=no");
                window.open("materialdosearchcontract.jsp?command=<%=Command.FIRST%>&txt_vendorname="+document.frmmaterial.txt_vendorname.value+"&frm_id=0","materialvendor", "height=500,width=800,status=yes,top, scrollbars=yes,toolbar=no,menubar=no,location=no");
	}
        ...
        <tr> 
          <td><!-- #BeginEditable "content" -->
            <form name="frmmaterial" method ="post" action="">
              ...
              <input type="hidden" name="hidden_contact_id" value="<%=vendorId%>">
              <input type="hidden" name="<%=FrmMatVendorPrice.fieldNames[FrmMatVendorPrice.FRM_FIELD_VENDOR_ID]%>" value="<%=vendorId%>">
             
              <input type="hidden" name="source_link" value="<%=sourceLink%>">
              <input type="hidden" name="hidden_contact_id" value="<%=vendorId%>">
               <input type="hidden" name="hidden_vendorContractId" value="<%=vendorContractId%>">
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <%}else{%>
                    <tr align="left" valign="top">
                      <td valign="middle" colspan="3"> 
                          <%= drawList(iCommand,listMatVendorPrice,-1,SESS_LANGUAGE)%>
                      </td>
                    </tr>
                <%
                  }
                  ...
                  <td height="22" colspan="3">
                    <table width="100%" border="0" cellspacing="1" cellpadding="1">
                      ...
                      <tr>
                        ...
                        <td width="1%">:</td>
                        <td width="37%"> 
                          <input type="text" size="30" readonly name="txt_vendorname" value="<%=material.getName()%>" class="formElemen">
                ...
                <tr align="left" valign="top"> 
                  <td height="22" valign="middle" colspan="3"> 
                   <%if((iCommand==Command.BACK || iCommand==Command.NONE ||iCommand==Command.FIRST || iCommand==Command.PREV || iCommand==Command.NEXT || iCommand==Command.LIST || iCommand==Command.LAST || iCommand==Command.SAVE || iCommand ==Command.DELETE) && frmMatVendorPrice.errorSize()==0){%> 
                ...
                <tr align="left" valign="top"> 
                  <td height="22" valign="middle" colspan="3"> 
                      <%
                                          ...
                                          %>
                                          <%--!--<a href="<%=cmdSelectToLgr%>" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image1003','','<%=approot%>/images/BtnNewOn.jpg',1)"><img src="<%=approot%>/images/BtnNew.jpg" alt="Select to Receive%>" name="Image1003" width="24" height="24" border="0" id="Image1003">Select to Receive</a>                                            <!--td valign="top" width="20%"-->
                                           --%>
                                          <%//} else {%>
