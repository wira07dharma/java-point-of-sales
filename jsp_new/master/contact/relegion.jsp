<%-- 
    Document   : relegion
    Created on : Apr 15, 2014, 4:17:51 PM
    Author     : dimata005
--%>

<%@page import="com.dimata.harisma.entity.masterdata.Religion"%>
<%@page import="com.dimata.harisma.entity.masterdata.PstReligion"%>
<%@page import="com.dimata.harisma.form.FrmReligion"%>
<%@page import="com.dimata.harisma.form.CtrlReligion"%>
...
{
	{"No","Agama","Nama Benua","Alamat","Telepon","Fax","Kontak Person","E-mail","Website","Locasi Induk","Tipe","Kontak Perusahaan","Keterangan", "Tax Persentase", "Service Percentase", "Tax & Service Standart","Persentasi Distribusi PO (%)","Nama Perusahaan"},
	{"No","Religion","Name","Address","Phone","Fax","Person Name","E-mail","Website","Parent Location","Type","Contact Link","Description", "Tax Persentase", "Service Percentase", "Tax & Service Default","Persentasi Distribusi PO (%)"}
};
...
public String drawList(int language, Vector objectClass , long negaraId, String approot, int start,int recordToGet)
{
	...
	for (int i = 0; i < objectClass.size(); i++)
	{
		...
		if(negaraId == religion.getOID())
			 index = i;
			 
		start = start + 1; 			
		rowx.add(""+start);
                
		rowx.add("<a href=\"javascript:cmdEdit('"+religion.getOID()+"')\">"+religion.getReligion()+"</a>");

		lstData.add(rowx);
                lstLinkData.add(String.valueOf(religion.getOID()));
...
int startReligion = FRMQueryString.requestInt(request, "start_negara");
int startMaterial = FRMQueryString.requestInt(request, "start_material");
int prevCommand = FRMQueryString.requestInt(request, "prev_command");
long oidReligion = FRMQueryString.requestLong(request, "hidden_negara_id");
...
String whereClause = "";
String orderClause = PstReligion.fieldNames[PstReligion.FLD_RELIGION];
...
CtrlReligion ctrlRelegion = new CtrlReligion(request); 
FrmReligion frmRelegion = ctrlRelegion.getForm(); 
iErrCode = ctrlRelegion.action(iCommand,oidReligion);
Religion relegion = ctrlRelegion.getReligion();
msgString =  ctrlRelegion.getMessage();
...
if(iCommand==Command.SAVE && iErrCode==FRMMessage.NONE){
	startReligion= PstReligion.findLimitStart(relegion.getOID(),recordToGet, whereClause);
}	
//update by fitra
int vectSize = PstReligion.getCount(whereClause);
if(iCommand==Command.FIRST || iCommand==Command.PREV || iCommand==Command.NEXT || iCommand == Command.LAST){
	startReligion= ctrlRelegion.actionList(iCommand, startReligion, vectSize, recordToGet);
} 
...
//update by fitra
listNegara = PstReligion.list(startReligion,recordToGet,whereClause,orderClause);
if(listNegara.size()<1 && startReligion>0){
	if(vectSize - recordToGet>recordToGet)
	 	startReligion= startReligion- recordToGet;   
  	else
	{
	 	startReligion= 0 ;
...
<%if((iCommand ==Command.ADD)||(iCommand==Command.SAVE)&&(frmRelegion.errorSize()>0)||(iCommand==Command.EDIT)||(iCommand==Command.ASK)){%>
	//window.location="#go";
<%}%>
...
function cmdAdd()
{
	...
	document.frmnegara.action="relegion.jsp";
	document.frmnegara.submit();
}

function cmdAsk(oidReligion)
{
	document.frmnegara.hidden_negara_id.value=oidReligion;
        ...
        <tr> 
          <td><!-- #BeginEditable "content" --> 
            <form name="frmnegara" method ="post" action="">
              ...
              <input type="hidden" name="start_negara" value="<%=startReligion%>">
              <input type="hidden" name="prev_command" value="<%=prevCommand%>">
              <input type="hidden" name="hidden_negara_id" value="<%=oidReligion%>">
              <input type="hidden" name="hidden_mat_minimum_id" value="<%=oidMinimum%>">			  
              <input type="hidden" name="<%=FrmReligion.FRM_FIELD_RELIGION%>" method="POST" action="">
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <tr align="left" valign="top"> 
                  <td height="8"  colspan="3"> 
                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
                      ...
                      <tr align="left" valign="top"> 
                        <td height="22" valign="middle" colspan="3"> <%= drawList(SESS_LANGUAGE,listNegara,oidReligion,approot,startReligion,recordToGet)%> </td>
                      </tr>
                      <tr align="left" valign="top"> 
                        <td height="8" align="left" colspan="3" class="command"> 
						  ...
						  ctrLine.initDefault();							
                          out.println(ctrLine.drawImageListLimit(cmd,vectSize,startReligion,recordToGet));
						  %> 
						  </span>
						</td>
                      </tr>
                          ...
                          <table border="0" cellspacing="1" cellpadding="1" width="100%">
                            <tr align="left"> 
                              ...
                              <td width="87%" valign="top"> 
                                <input type="text" name="<%=frmRelegion.fieldNames[FrmReligion.FRM_FIELD_RELIGION] %>"  value="<%= relegion.getReligion() %>" class="formElemen" size="20">
                                * <%= frmRelegion.getErrorMsg(FrmReligion.FRM_FIELD_RELIGION) %> </td>
                            </tr>
                          </table>
                        </td>
                      </tr>
