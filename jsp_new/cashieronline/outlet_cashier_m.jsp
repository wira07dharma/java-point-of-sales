<%-- 
    Document   : outlet_cashier_m
    Created on : Sep 11, 2014, 1:15:03 PM
    Author     : dimata005
--%>
...
*/
public String drawListPoItem(int language,int iCommand,Vector objectMaterial,FrmBillDetail frmBillDetail, long oidOpenBill,Vector objectMaterialIssue) {
    ...
    //ctrlist.addHeader("<center>"+textListOrderItem[language][8]+"</center>","5%");//unit
    ...
    rowx = new Vector();
    if(objectMaterial.size()!=0 && objectMaterialIssue.size()==0){
        int count=0;
        for(int i=0; i<objectMaterial.size(); i++) {
            
            
            Vector vBillDetail =(Vector) objectMaterial.get(i);
            ...
            count=count+1;
            rowx = new Vector();
            rowx.add(""+count+"<input type=\"hidden\" name=\""+frmBillDetail.fieldNames[frmBillDetail.FRM_FIELD_MATERIAL_ID]+"_"+i+"\" value=\""+billdetail.getMaterialId()+"\">");
            rowx.add("<div align=\"left\">"+billdetail.getSku()+"</div><input type=\"hidden\" name=\""+frmBillDetail.fieldNames[frmBillDetail.FRM_FIELD_SKU]+"_"+i+"\" value=\""+billdetail.getSku()+"\">");
            rowx.add("<div align=\"left\">"+billdetail.getItemName()+"</div><input type=\"hidden\" name=\""+frmBillDetail.fieldNames[frmBillDetail.FRM_FIELD_ITEM_NAME]+"_"+i+"\" value=\""+billdetail.getItemName()+"\">");
            ...
            rowx.add("<div align=\"center\"><input type=\"hidden\" name=\""+frmBillDetail.fieldNames[frmBillDetail.FRM_FIELD_QTY]+"_"+i+"\" value=\""+billdetail.getQty()+"\">"+billdetail.getQty()+"</div>");
            rowx.add("<div align=\"center\"><input type=\"hidden\" name=\""+frmBillDetail.fieldNames[frmBillDetail.FRM_FIELD_UNIT_ID]+"_"+i+"\" value=\""+billdetail.getUnitId()+"\">"+unit.getCode()+"</div>");
           /* double qtyKirim = 0;
            qtyKirim = PstBillMain.getStockOrder(oidOpenBill,  billdetail.getMaterialId());
            rowx.add("<div align=\"center\">"+qtyKirim+"</div>");*/
            
            double qtyIssue=0;
            qtyIssue=billdetail.getQty();//-qtyKirim;
            rowx.add("<div align=\"center\"><input type=\"text\" name=\""+frmBillDetail.fieldNames[frmBillDetail.FRM_FIELD_QTY_ISSUE]+"_"+i+"\" value=\""+qtyIssue+"\" style=\"text-align:right\" onclick=\"javascript:changeAll(this.value,'"+billdetail.getMaterialId()+"',"+i+","+billdetail.getQty()+")\" onkeyup=\"javascript:changeQty(this.value,'"+billdetail.getMaterialId()+"',"+i+","+billdetail.getQty()+")\"></div>");
...
if (oidOpenbillmain != 0 && oidBillMain==0) {
  ...
  iErrCode = ctrlBillDetail.action(iCommand,oidBillMain,request,lisBillDetail,customerId,priceTypeId,oidOpenbillmain);  
...
<html>
    ...
    <body class="skin-blue">
        ...
        <div class="wrapper row-offcanvas row-offcanvas-left">
            ...
            <aside class="right-side" style="margin-left:10px; margin-right:10px;">
                ...
                <section class="content">
                    <form name="frmcashier" method ="post" action=""  role="form">
                        ...
                        <input type="hidden" name="<%=frmBillMain.fieldNames[frmBillMain.FRM_FIELD_CASH_CASHIER_ID]%>" value="<%=oidCashCashierOnline%>">
                        ...
                        <div class="box-body">
                            <div class="row">
                                ...
                                <div class="col-xs-12">
                                    <%= drawListPoItem(SESS_LANGUAGE,iCommand,lisBillDetail,frmBillDetail, oidOpenbillmain,lisBillDetailIssue)%> 
                                </div>
                                <div class="col-xs-12">
                                     <div class="box-footer">
                                         <button id="btnPrint" type="button" class="btn btn-success pull-right nonprint">Print</button>
                                         <%--
                                            <button class="btn btn-danger pull-left" style="margin-right: 5px;" onclick="javascript:cmdSave()" ><i class="fa fa-download"></i> Save</button>
                                            <button class="btn btn-warning pull-left" style="margin-right: 5px;" onclick="javascript:cmdBack()" ><i class="fa fa-download"></i> Back</button>
                                           --%>
                                    </div>
                                </div>
                            </div>
