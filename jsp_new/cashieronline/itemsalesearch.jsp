...
<%!    /* this constant used to list text of listHeader */
    public static final String textListMaterialHeader[][] = {
        {"N0.", "SKU", "Nama Barang", "Unit", "Qty Sales", "Harga", "Gudang", "QTY Invoicing", "Total Qty Outstanding"},
        {"No.", "SKU", "Material Name", "Unit", "Qty Sales", "Price", "Warehouse", "QTY Invoicing","Total Qty Outstanding"}
    };
    ...
    public String drawListMaterial(long oidBillMain, int language, Vector objectClass, int start, Vector vectLocWh) {
        ...
        if (objectClass != null && objectClass.size() > 0) {
            ...
            for (int i = 0; i < objectClass.size(); i++) {
                ...
                rowx.add(billdetail.getItemName() + "<input type=\"hidden\" name=\"itemName"+i+"\" value=\""+billdetail.getItemName()+"\">"); // update by McHen [2014-12-22]
                ...
                double stockWh = 0;
                if(totalQty <= 0){
                    rowx.add("Outstanding = 0");
                } else {
                    if (vectLocWh != null && vectLocWh.size() > 0) {
                        // 2014-12-19 update by mchen
                        rowx.add("<div align=\"center\">"
                                + "<input tabindex=\"33\" type=\"text\" size=\"3\" name=\"matqty"+i+"\" value=\"\" class=\"formElemenR\" style=\"text-align:right\" />"
                                + "</div>");
                        /*
                        for (int k = 0; k < vectLocWh.size(); k++) {
                            Location location = (Location) vectLocWh.get(k);
                            ...
                            rowx.add("<div align=\"center\">" + stockWh
                                    + "<input tabindex=\"33\" type=\"text\" size=\"3\" name=\"matqty[" + k + "]\" value=\"\" class=\"formElemenR\" style=\"text-align:right\" />"
                                    + "<input type=\"hidden\" id=\"idx\" name=\"idx\" value=\""+k+"\" />"
                                    + "<input type=\"hidden\" id=\"qty\" name=\"qty\" value=\""+billdetail.getQty()+"\" />"
                                    + "<input type=\"hidden\" id=\"qtyOrder\" name=\"qtyOrder\" value=\""+qtyOrder+"\" />"
                                    + "</div>");
                        }
                        */
                    }
...
<%
            ...
            // mchen | start declare variable           
            String pesan1 = "";
            String pesan2 = "";
            double credit_limit = FRMQueryString.requestDouble(request, "creditlimit");
            double out_standing = FRMQueryString.requestDouble(request, "outstanding");
            CtrlMemberReg ctrlMemberReg = new CtrlMemberReg(request);               
            long oidMemberReg = FRMQueryString.requestLong(request, "oidMemberReg");
            // mchen | end declare variable
            ...
            if (iCommand == Command.GOTO) {
                ...
                for (int i = 0; i < listBillDetail.size(); i++) {
                    ...
                    String qtyInput = FRMQueryString.requestString(request, "matqty"+i);
                    // 2014-12-19 update by mchen
                    //for (int j = 0; j < vectLocWer.size(); j++) {
                        //String qtyAvailable = FRMQueryString.requestString(request, "matqty" + i + "");///
                        
                        try{
                            stockQty = Double.parseDouble(qtyInput);
                        }catch(Exception e){
                            stockQty =0;
                        }
                    ...
                    // 2014-12-19 update by mchen
                    if (stockQty > qtyOrderSales){
                        stockAvailable = 0;
                        pesan1 = "Melebihi Stock";
                        %>
                        <script>
                        ...
                        if ((qtyOrder+stockQty) > qtyOrderSales){
                            stockAvailable = 0;
                            pesan1 = "Melebihi Stock";
                            %>
                            <script>
                            ...
                            double totalPriceTemp = stockAvailable * netPrice;
                            totalPrice = totalPrice + out_standing;//
                            if (totalPrice > credit_limit){
                                pesan2 = "Total price melebihi credit limit ";
                                //insertHistory(long userID, String nameUser, Command.SAVE, long oid);
                                //ctrlMemberReg.insertHistory(1, "abc", Command.SAVE, 2);//
                                
                                String itemName = FRMQueryString.requestString(request, "itemName"+i);
                                try {
                                    ctrlMemberReg.insertHistoryInvoice(userId, userName, Command.SAVE, oidMemberReg, itemName);
                                } catch (Exception exc) {
                                ...
                                if(stockAvailable>0){
                                    ...
                                    billdetail.setTotalPrice(totalPriceTemp);
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdConfrmApprove(){
                ...
                //checkValue(changeVal, qty, stockVal, idx);
            ...
            function checkValue(changeVal, qty, stockVal, idx){
                ...
                var stockValf = parseFloat(stockVal);
                var qtyOrder=parseFloat(qty);
                var i = parseInt(idx);
                //var nameVariable="matqty[" + idx + "]";
                if(inputVal>qtyOrder || inputVal>stockValf){
                    //alert("sorry,  value input must less than or same as qty order");
                    document.frmstockcodedosearch.matqty[i].value="0";
                }
            }
                        ...
                        <tr>
                            <td>
                                <form name="frmstockcodedosearch" method="post" action="">
                                    ...
                                    <input type="hidden" name="oidMemberReg" value="<%=oidMemReg%>">
                                    <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                        ...
                                        <tr>
                                            <td colspan="2">
                                                <div style="background-color:#eaff00;color:#ff0000; padding: 5px; font-size: 12px; font-weight: bold;">
                                                    <%=pesan1%>
                                                    <br />
                                                    <%=pesan2%>
                                                </div>
