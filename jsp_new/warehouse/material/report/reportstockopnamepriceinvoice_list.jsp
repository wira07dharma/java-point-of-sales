...
{
	{"No","Tanggal","SKU","Nama","Harga Jual","Qty Opn","Qty Sld","Qty Sys","Selisih","Total Lost"},
	{"No","Date","Code","Name","Price","Opn Qty","Sld Qty","Sys Qty","Blnc","Total Lost"}
};
						...
						boolean isSubCategory,boolean isSupplier)
{
	...
	if(objectClass!=null && objectClass.size()>0)
	{
		...
		ctrlist.dataFormat("","3%","0","0","right","bottom");
		ctrlist.dataFormat("","11%","0","0","right","bottom");
		ctrlist.dataFormat("","18%","0","0","right","bottom");
		ctrlist.dataFormat("","1%","0","0","right","bottom");
		ctrlist.dataFormat("","5%","0","0","right","bottom");
		ctrlist.dataFormat("","1%","0","0","right","bottom");
		ctrlist.dataFormat("","5%","0","0","right","bottom");
		ctrlist.dataFormat("","1%","0","0","right","bottom");
		ctrlist.dataFormat("","5%","0","0","right","bottom");
		...
		ctrlist.dataFormat("","1%","0","0","right","bottom");
		ctrlist.dataFormat("","13%","0","0","right","bottom");
		ctrlist.dataFormat("","1%","0","0","right","bottom");
		...
		for(int i=0; i<objectClass.size(); i++) 
		{
			...
			MatStockOpname so = (MatStockOpname)vt.get(0);
			MatStockOpnameItem soi = (MatStockOpnameItem)vt.get(1);
			...
			if (noBill != so.getOID())
			{
				...
				if (firstRow == true)
				{
					...
					rowx.add("|");
					rowx.add("Tanggal");
					rowx.add("<div align=\"left\">&nbsp;:&nbsp;"+Formater.formatDate(so.getStockOpnameDate(), "dd-MM-yyyy")+"</div>");
					rowx.add("|");
					lstData.add(rowx);
					baris += 1;
					//Add invoice number
					...
					rowx.add("|");
					rowx.add("Invoice");
					rowx.add("<div align=\"left\">&nbsp;:&nbsp;"+so.getStockOpnameNumber()+"</div>");
					rowx.add("|");
					lstData.add(rowx);
					baris += 1;
					lstData.add(drawLineHorizontal());
			...
			rowx.add(unit_name);
			rowx.add("<div align=\"center\">"+"|"+"</div>");
			rowx.add("<div align=\"right\">"+FRMHandler.userFormatStringDecimal(soi.getPrice())+"</div>");			
			rowx.add("<div align=\"center\">"+"|"+"</div>");
			rowx.add("<div align=\"right\">"+FRMHandler.userFormatStringDecimal(qtyLost * soi.getPrice())+"</div>");
			rowx.add("|");
			lstData.add(rowx);
			baris += 1;
				if (baris == 72)
			...
			totalLost += (soi.getPrice() * qtyLost);
			totalQty += qtyLost;
			subTotalLost += (soi.getPrice() * qtyLost);
			subTotalQty += qtyLost;
...
*/
Vector records = sessReportStockOpname.getReportStockOpname(srcReportStockOpname);
...
function cmdListFirst()
{
	...
	document.frm_reportstockopname.action="reportstockopnameinvoice_list.jsp";
...
function cmdListNext()
{
	...
	document.frm_reportstockopname.action="reportstockopnamepriceinvoice_list.jsp";
	document.frm_reportstockopname.submit();
}
...
function cmdBack()
{
	...
	document.frm_reportstockopname.action="src_reportstockopnameinvoice.jsp";
	document.frm_reportstockopname.submit();
}

function printForm(){
	window.open("reportstockopnamepriceinvoice_form_print.jsp","stockreport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
}
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr> 
          <td height="20" class="mainheader"><!-- #BeginEditable "contenttitle" --> 
            Stock Opname Report by Invoice &gt; Item List <!-- #EndEditable --></td>
        </tr>
        <tr> 
			...
			%>
                ...
                <tr align="left" valign="top"> 
                  <td height="22" valign="middle" colspan="3">
					<%
					Vector hasil = drawList(SESS_LANGUAGE,records,isCategory,isSubCategory,isSupplier);
					Vector report = new Vector(1,1);
					report.add(srcReportStockOpname);
					report.add(hasil);
					try
					{
						session.putValue("SESS_MAT_STOCK_OPNAME_REPORT_INVOICE",report);
					}
					catch(Exception e){}
					%>
				  </td>
