...
public static final String textListGlobal[][] = {
	{"Laporan Pembiayaan Global","Pembiayaan","Laporan Per Nota","Pencarian","Cetak Laporan Pembiayaan","Tidak ada data pembiayaan",
	 "Laporan Pembiayaan"},
	{"Global Costing Report","Costing","Invoice Report","Search","Print Costing Report","No costing data available",
	 "Costing Report"}
};

/* this constant used to list text of listHeader */
public static final String textListHeader[][] = {
	{"Periode"," s/d ","Lokasi Asal","Lokasi Tujuan","Kategori","Urut Berdasar","Sub Kategori","Suplier", "Tipe Biaya","Semua Lokasi"},
	{"Period"," to ","From Location","To Location","Category","Sort By","Sub Category","Supplier", "Type of Costing", "All Location"}
};
...
/** get type costing list */
Vector costingGroup = PstCosting.list(0,0,"",PstCosting.fieldNames[PstCosting.FLD_NAME]);
Vector vectGroupVal = new Vector(1,1);
Vector vectGroupKey = new Vector(1,1);
vectGroupVal.add(textListHeader[SESS_LANGUAGE][9]+" "+textListHeader[SESS_LANGUAGE][8]);
vectGroupKey.add("0");
if(costingGroup != null && costingGroup.size() > 0) {
	for(int i=0; i<costingGroup.size(); i++) {
		Costing cost = (Costing)costingGroup.get(i);
		vectGroupVal.add(cost.getName());
		vectGroupKey.add(""+cost.getOID());
	}
}
...
function cmdSearch() {
	...
	document.frmsrcdispatchreport.action="reportcostinginvoice_list.jsp";
				...
				<tr>
                  <td colspan="2">
                    <table width="100%" border="0" cellspacing="1" cellpadding="1">
                      ...
                      <tr>
                        <td height="21" width="9%"><%=getJspTitle(8,SESS_LANGUAGE,"",true)%></td>
                        <td height="21" width="1%">:</td>
                         <td height="21" width="87%">
                          <% //String where = PstLocation.fieldNames[PstLocation.FLD_LOCATION_ID]+
                                          // " != "+oidLocation;
                                ...
                                Vector vt_cost = PstCosting.list(0, 0, "", PstCosting.fieldNames[PstCosting.FLD_NAME]);
                                    ...
                                    //  else
                                    //        key_locationid.add("All Location");
					...
					key_costingid.add(cost.getName());
                          ...
                          %>
                          <%=controlCheckBox.draw(frmSrcReportDispatch.fieldNames[frmSrcReportDispatch.FRM_FIELD_COSTING_ID],val_costingid,key_costingid,new Vector())%>
                         <%//=ControlCombo.draw(frmSrcReportDispatch.fieldNames[frmSrcReportDispatch.FRM_FIELD_COSTING_ID],"formElemen", null, ""+srcReportDispatch.getCostingId(), vectGroupKey, vectGroupVal, null)%></td>
                      </tr>
