...
<% int  appObjCode = AppObjInfo.composeObjCode(AppObjInfo.G1_TRANSFER, AppObjInfo.G2_TRANSFER_REPORT, AppObjInfo.OBJ_TRANSFER_REPORT_BY_CATEGORY); %>
...
public static final String listTextGlobal[][] = {
	{"LAPORAN TRANSFER BARANG PER KATEGORI","Transfer","Laporan Per Kategori","Pencarian",
	 "Cetak Laporan Transfer"," Tidak ada data transfer barang...","Laporan Transfer"},
	{"GOODS TRANSFER REPORT BY CATEGORY","Transfer","By Category Report","Search",
	 "Print Transfer Report","There is no goods transfer data ...","Transfer Report"}
};

public static final String listTextTitle[][] = {
	{"Periode"," s/d ","Lokasi Asal","Lokasi Tujuan","Suplier","Kategori","Semua"},
	{"Period"," to ","From Location","To Location","Supplier","Category","All"}
};
...
{
	{"Lokasi Asal ","Kategori","Lokasi Tujuan","LAPORAN PENGIRIMAN BARANG PER KATEGORI"},
	{"From Location ","Category","To Location","REPORT DISPATCH ITEM PER CATEGORY"}
};
						...
						boolean isDispatchTo, boolean isSupplier)
{
	...
	if(objectClass!=null && objectClass.size()>0)
	{
		...
		for(int i=0; i<objectClass.size(); i++)
		{
			...
			totalQty += dfi.getQty();
			subTotalPrice += (dfi.getQty() * dfi.getHpp());
...
*/
Vector records = new Vector(); // sessReportDispatch.getReportDispatchTotal(srcReportDispatch);
...
function cmdListFirst()
{
	...
	document.frm_reportdispatch.action="reportdispatchkategori_list.jsp";
...
function cmdBack()
{
	...
	document.frm_reportdispatch.action="src_reportdispatchkategori.jsp";
	document.frm_reportdispatch.submit();
}

function printForm(){
	window.open("reportdispatchkategori_form_print.jsp","dispatchkategorireport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
}
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">  
        <tr> 
          <td height="20" class="mainheader"><!-- #BeginEditable "contenttitle" --> 
            Laporan Pengiriman Barang Per Kategori<!-- #EndEditable --></td>
        </tr>
        <tr> 
          <td><!-- #BeginEditable "content" --> 
            <form name="frm_reportdispatch" method="post" action="">
              ...
              <table width="100%" cellspacing="0" cellpadding="3">
                ...
                <tr align="left" valign="top"> 
                  <td height="14" colspan="3" align="center" valign="middle" ><strong>
                    <%=textHeader[SESS_LANGUAGE][3]%></strong></td>
                </tr>
                <tr align="left" valign="top"> 
                  <td height="14" colspan="3" align="center" valign="middle" ><%=Formater.formatDate(srcReportDispatch.getDateFrom(), "dd-MM-yyyy")%> <%= " s/d " + Formater.formatDate(srcReportDispatch.getDateTo(), "dd-MM-yyyy")%></td>
                </tr>
                <%
                    Vector vLSearch = new Vector();
                    if(srcReportDispatch.getDispatchTo()==0){
                        String locWhereClause = PstLocation.fieldNames[PstLocation.FLD_TYPE] + " = " + PstLocation.TYPE_LOCATION_STORE;
                        String locOrderBy = PstLocation.fieldNames[PstLocation.FLD_CODE];
                        Vector vt_loc1 = PstLocation.list(0,0,"",locOrderBy);
                        for(int i=0;i<vt_loc1.size();i++){
                            SrcReportDispatch srcReportDf = new SrcReportDispatch();
                            Location loc = (Location)vt_loc1.get(i);

                            srcReportDf.setDispatchTo(loc.getOID());
                            srcReportDf.setCategoryId(srcReportDispatch.getCategoryId());
                            srcReportDf.setDateFrom(srcReportDispatch.getDateFrom());
                            srcReportDf.setDateTo(srcReportDispatch.getDateTo());
                            srcReportDf.setLocationId(srcReportDispatch.getLocationId());
                            srcReportDf.setSortBy(srcReportDispatch.getSortBy());
                            srcReportDf.setSubCategoryId(srcReportDispatch.getSubCategoryId());
                            srcReportDf.setSupplierId(srcReportDispatch.getSupplierId());

                            vLSearch.add(srcReportDf);
                        }
                    }else{
                        vLSearch.add(srcReportDispatch);
                    }

                    for(int i=0;i<vLSearch.size();i++){
                        srcReportDispatch = (SrcReportDispatch)vLSearch.get(i);

                        // proses pengambilan data dispatch
                        records = sessReportDispatch.getReportDispatchTotal(srcReportDispatch);
                        vectSize = records.size();
                        ...
                        if (srcReportDispatch.getCategoryId() != 0){
                            try{
                                kategori = PstCategory.fetchExc(srcReportDispatch.getCategoryId());
                            }catch(Exception exx){}
                        }
                ...
                <tr align="left" valign="top"> 
                  <td height="14" valign="middle" colspan="3" class="command"> 
                      <b><%=textHeader[SESS_LANGUAGE][0]%> : <%=lokasi.getName()%> </b> </td>
                </tr>
                <tr align="left" valign="top">
                  <td height="14" valign="middle" colspan="3"><strong><%=textHeader[SESS_LANGUAGE][1]%>
                    : <%=kategori.getName()%></strong></td>
                </tr>
                <%}%>
                <tr align="left" valign="top">
                  <td height="14" valign="middle" colspan="3"><strong><%=textHeader[SESS_LANGUAGE][2]%> :
                    <%=loc1.getName()%> </strong></td>
                </tr>
                <tr align="left" valign="top">
                  ...
                  <td height="18" valign="top" colspan="3"> <table width="100%" border="0">
                      <tr> 
                        <td width="66%"> <table width="52%" border="0" cellspacing="0" cellpadding="0">
                            <tr> 
                              <!--td nowrap width="7%"><a href="javascript:cmdBack()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image10','','<%=approot%>/images/BtnBackOn.jpg',1)" id="aSearch"><img name="Image10" border="0" src="<%=approot%>/images/BtnBack.jpg" width="24" height="24" alt="<%=ctrLine.getCommand(SESS_LANGUAGE,"Laporan Pengiriman Barang",ctrLine.CMD_BACK_SEARCH,true)%>"></a></td-->
                              <td nowrap width="3%">&nbsp;</td>
                              <td class="command" nowrap width="90%"><a class="btn btn-primary" href="javascript:cmdBack()"><i class="fa fa-arrow-left"></i> <%=ctrLine.getCommand(SESS_LANGUAGE,"Laporan Pengiriman Barang",ctrLine.CMD_BACK_SEARCH,true)%></a></td>
                            </tr>
                          </table></td>
                        <td width="34%"> 
