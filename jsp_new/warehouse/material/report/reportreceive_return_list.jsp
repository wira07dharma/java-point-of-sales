...
if(iCommand==Command.BACK || iCommand==Command.FIRST || iCommand==Command.PREV || iCommand==Command.NEXT || iCommand==Command.LAST) {
	 ...
	 session.putValue(SessReportReceive.SESS_SRC_REPORT_RECEIVE_RETURN, srcReportReceive);
}
...
function cmdBack() {
	...
	document.frm_reportsale.action="src_reportreceive_return.jsp";
	document.frm_reportsale.submit();
}

function printForm() {
	window.open("reportreceive_return_list_html.jsp","allreceivereportall","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
...
function printFormExcel(){
    window.open("reportreceive_return_list_excel.jsp");
}
