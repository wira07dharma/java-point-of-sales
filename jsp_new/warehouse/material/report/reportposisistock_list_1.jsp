...
public static final String textListTitleTable[][] = {
    {
        "NO","SKU","NAMA BARANG","HARGA BELI","UNIT","JUMLAH STOK","AWAL","OPNAME","TERIMA","TRANSFER","RETUR SUPP.",//10
        "JUAL","RETUR CUST.","SALDO","NILAI STOK","HARGA JUAL","BARCODE","BIAYA","KATEGORI","SEASON","PRODUKSI"//20
    },
    {
        "NO","SKU","GOODS NAME","COGS","UNIT","QUANTITY STOCK","BEGINNING","OPNAME","RECEIVE","DISPATCH","SUPP. RETURN",//10
        "SALE","CUST. RETURN","SALDO","STOCK VALUE","SALE PRICE","BARCODE","COST","CATEGORY","SEASON","PRODUCTION"//20
    }
};
...
if(isSummarryTransactionData) { // cek apakah proses rekap data OK
    ...
    if(iViewType == 0) {
      //vectSize = objSessReportPotitionStock.getCountReportStockAll(srcReportPotitionStock, isCalculateZeroQty, 0,0);
    }
...
String masterSeason = PstSystemProperty.getValueByName("SEASON_GROUP_TYPE");    
...
function printFormExcel() {
    //window.open("reportstock_form_print.jsp","stockposisireport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
    window.open("reportposisistock_list_1_excel.jsp?report_title1=<%=textListGlobal[SESS_LANGUAGE][1]%>&report_title2=<%=reportTilte%>&FRM_FIELD_PRICE_TYPE_ID=<%=srcReportPotitionStock.getPriceTypeId()%>&view_type=<%=iViewType%>","form_stockpotitionreport");
}

function printFormHtml() {
    //window.open("reportstock_form_print.jsp","stockposisireport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
    window.open("reportposisistock_list_1_html.jsp?report_title1=<%=textListGlobal[SESS_LANGUAGE][1]%>&report_title2=<%=reportTilte%>&FRM_FIELD_PRICE_TYPE_ID=<%=srcReportPotitionStock.getPriceTypeId()%>&view_type=<%=iViewType%>","form_stockpotitionreport");
}
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">  
		...
		<tr align="left" valign="top">
                          ...
                          <td>
                            ...
                            %>  
                              <tr>
									...
									<td width="12%" class="listgentitle" rowspan="2" colspan="0" ><%=textListTitleTable[SESS_LANGUAGE][18]%></td>
									
									<%if(!masterSeason.equals("0") && masterSeason.length()>0){%>
                                        <td width="7%" class="listgentitle" rowspan="2" colspan="0" ><%=textListTitleTable[SESS_LANGUAGE][19]%></td>
                                    <%}%>
                                    <%
                                    ...
                                    <% if(infoShowed == SrcReportPotitionStock.SHOW_QTY_ONLY || infoShowed == SrcReportPotitionStock.SHOW_BOTH) { %>
                                        ...
                                        <td width="0%"  class="listgentitle" rowspan="" colspan="<%=colspan%>" align="center"><%=textListTitleTable[SESS_LANGUAGE][5]%></td>
                                    <% } %>
                                    
                                    
                                    <%if(infoShowed == SrcReportPotitionStock.SHOW_VALUE_ONLY){ %>

                                    <td width="0%"  class="listgentitle" rowspan="" colspan="<%=colspan%>" ><%=textListTitleTable[SESS_LANGUAGE][14]%></td>
                                    <% } %>
                                    <% if(infoShowed == SrcReportPotitionStock.SHOW_BOTH) { %>
                                    ...
                                    <tr>
                                      ...
                                      <td width="7%" class="listgentitle" ><%=textListTitleTable[SESS_LANGUAGE][20]%></td>
                                      <td width="7%" class="listgentitle" ><%=textListTitleTable[SESS_LANGUAGE][13]%></td>
                                    </tr>
                                   
                                    <%if(iViewType == 0) {%>
                                        <% objSessReportPotitionStock.getReportStockAll(out, infoShowed, stockValueBy, SESS_LANGUAGE, start, srcReportPotitionStock,isCalculateZeroQty, start, recordToGet, cellStyle);%>
                                    <%}else {%>
                                        <% objSessReportPotitionStock.getGrandTotalReportStockAll(out, infoShowed, stockValueBy, SESS_LANGUAGE, start, srcReportPotitionStock,isCalculateZeroQty, 0, 0, cellStyle);%>
                                    <%}%>
			  ...
			  <table width="100%" border="0" cellpadding="0" cellspacing="0">
                            <tr> 
                              <!--td width="5%" valign="top"><a href="javascript:printFormHtml()"><img src="<%=approot%>/images/BtnPrint.gif" width="31" height="27" border="0" alt="<%=textListGlobal[SESS_LANGUAGE][10]%>"></a></td-->
                              <td width="35%" nowrap>&nbsp; <a class="btn btn-primary" href="javascript:printFormHtml()" class="command" ><i class="fa fa-print"></i> <%=textListGlobal[SESS_LANGUAGE][10]%> Html</a></td>  
                              <%--  
                              ...
                              <td width="25%" nowrap>&nbsp; <a class="btn btn-primary" href="javascript:printFormExcel()" class="command" ><i class="fa fa-print"></i> <%=textListGlobal[SESS_LANGUAGE][10]%> Excel</a></td>
