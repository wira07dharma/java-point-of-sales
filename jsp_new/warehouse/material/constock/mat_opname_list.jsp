...
public String drawList(Vector objectClass, int start, int language, int docType, I_DocStatus i_status)
{
	...
	{	
		...
		for (int i = 0; i < objectClass.size(); i++) 
		{
			...
			start = start + 1;			
			MatConStockOpname matStockOpname = (MatConStockOpname)vt.get(0);
		...
		result = "<div class=\"msginfo\">&nbsp;&nbsp;Tidak ada dokumen opname barang ...</div>";
	}
	return result;	
}
%>
...
long oidMatConStockOpname = FRMQueryString.requestLong(request, "hidden_mat_stock_opname_id");
...
FrmSrcMatConStockOpname frmSrcMatConStockOpname = new FrmSrcMatConStockOpname(request, srcMatConStockOpname);
frmSrcMatConStockOpname.requestEntityObject(srcMatConStockOpname);
...
SessMatConStockOpname sessMatConStockOpname = new SessMatConStockOpname();
session.putValue(SessMatConStockOpname.SESS_SRC_MATSTOCKOPNAME, srcMatConStockOpname);
vectSize = sessMatConStockOpname.getCountSearch(srcMatConStockOpname);

ctrlMatConStockOpname.action(iCommand , oidMatConStockOpname);
if((iCommand==Command.FIRST)||(iCommand==Command.NEXT)||(iCommand==Command.PREV)||(iCommand==Command.LAST)||(iCommand==Command.LIST)){
	start = ctrlMatConStockOpname.actionList(iCommand, start, vectSize, recordToGet);
}

Vector records = sessMatConStockOpname.searchMatConStockOpname(srcMatConStockOpname, start, recordToGet);
try{
        ...
        <tr> 
          <td><!-- #BeginEditable "content" --> 
            <form name="frm_matstockopname" method="post" action="">
              ...
              <input type="hidden" name="hidden_opname_id" value="<%=oidMatConStockOpname%>">
                  ...
                  <td> 
                    <table width="52%" border="0" cellspacing="0" cellpadding="0">
                      <tr> 
                        ...
                        <td class="command" nowrap width="48%"><a href="javascript:cmdAdd()"><%=ctrLine.getCommand(SESS_LANGUAGE,opnameTitle,ctrLine.CMD_ADD,true)%></a></td>
                        <%//}%>
                        ...
                        <td class="command" nowrap width="40%"><a href="javascript:cmdBack()"><%=ctrLine.getCommand(SESS_LANGUAGE,opnameTitle,ctrLine.CMD_BACK_SEARCH,true)%></a></td>
                      </tr>
                    </table>
                  </td>
                </tr>
