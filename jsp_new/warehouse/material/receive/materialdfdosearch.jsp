...
public long getOidRecItem(Vector vectRec, long oidmaterial){
	...
	if(vectRec!=null && vectRec.size()>0){
		for(int k=0;k<vectRec.size();k++){
			MatReceiveItem matRec = (MatReceiveItem)vectRec.get(k);
...
public String drawListMaterial(int language,Vector objectClass, Vector vectRec, int start, int tranUsedPriceHpp, boolean privShowQtyPrice){
	String result = "";
	if(objectClass!=null && objectClass.size()>0){
		ControlList ctrlist = new ControlList();
		ctrlist.setAreaWidth("100%"); 
		...
		for(int i=0; i<objectClass.size(); i++){
			...
			MatDispatchItem matDispatchItem = (MatDispatchItem)objectClass.get(i);
			...
			Unit unit = new Unit();
			try{
				unit = PstUnit.fetchExc(matDispatchItem.getUnitId());//PstUnit.fetchExc(material.getDefaultStockUnitId());
			}catch(Exception e){}
...
/*
String whereClause = " DFI." + PstMatDispatchItem.fieldNames[PstMatDispatchItem.FLD_DISPATCH_MATERIAL_ID] +
		" = " + oidDispatch;
Vector vect = PstMatDispatchItem.list(whereClause,orderClause);
*/

String whereClause = PstMatReceiveItem.fieldNames[PstMatReceiveItem.FLD_RECEIVE_MATERIAL_ID]+"="+matReceiveOid;
Vector vectRec = PstMatReceiveItem.list(0,0,whereClause,""); 

whereClause = PstMatDispatch.fieldNames[PstMatDispatch.FLD_DISPATCH_MATERIAL_ID] + " = " + oidDispatch;
Vector vect = com.dimata.posbo.entity.warehouse.PstMatDispatchItem.list(0,0,whereClause,"");
...
function cmdEdit(matOid,matCode,matItem,matUnit,matPrice,matUnitId,qtyDF,matCurrencyId,matCurrCode,matTotal,oid){	
	if(oid==0){
		...
		self.opener.document.forms.frm_recmaterial.<%=FrmMatReceiveItem.fieldNames[FrmMatReceiveItem.FRM_FIELD_QTY]%>.value = qtyDF;
		self.opener.document.forms.frm_recmaterial.<%=FrmMatReceiveItem.fieldNames[FrmMatReceiveItem.FRM_FIELD_TOTAL]%>.value = matTotal;	
		self.opener.document.forms.frm_recmaterial.<%=FrmMatReceiveItem.fieldNames[FrmMatReceiveItem.FRM_FIELD_QTY]%>.focus();		
	}else{
		self.opener.document.forms.frm_recmaterial.hidden_receive_item_id.value = oid;
		self.opener.document.forms.frm_recmaterial.command.value = "<%=Command.EDIT%>";
          ...
          <td> 
            <form name="frmvendorsearch" method="post" action="">
              ...
              <table width="100%" border="0" cellspacing="1" cellpadding="1">
                ...
                <tr> 
                  <td colspan="2">
				  <%=drawListMaterial(SESS_LANGUAGE,vect,vectRec,start,tranUsedPriceHpp,privShowQtyPrice)%>
				  </td>
                </tr>
                <tr> 
