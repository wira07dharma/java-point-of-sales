...
static int checkingStock = Integer.valueOf(PstSystemProperty.getValueByName("CHECKING_STOCK"));

public static final String textListGlobal[][] = {
	{"Pencarian Nota Penerimaan"},
	{"Searching Receipt"}
};
...
public String drawList(int language,Vector objectClass, int start) {
	...
	if(objectClass!=null && objectClass.size()>0){
		...
		for(int i=0; i<objectClass.size(); i++) 
		{
			...
			lstData.add(rowx);
			lstLinkData.add(rec.getInvoiceSupplier()+"', '"+rec.getOID()+"', '"+rec.getPurchaseOrderId()+"', '"+rec.getLocationId()+"', '"+rec.getCurrencyId());
		}
		result = ctrlist.draw();
	}
	else
...
<%
	...
	String orderClause = PstMatReceive.fieldNames[PstMatReceive.FLD_INVOICE_SUPPLIER];
        ...
        if(checkingStock==SessStockCard.CHECKING_STOCK_ALLOW_MINUS){
              whereClasue = PstMatReceive.fieldNames[PstMatReceive.FLD_TRANSFER_STATUS]+"="+I_DocStatus.DOCUMENT_STATUS_DRAFT+
                            ...
                            //+where locationId
                            " AND "+PstMatReceive.fieldNames[PstMatReceive.FLD_LOCATION_ID]+"="+locationId;
        }else{
              whereClasue = PstMatReceive.fieldNames[PstMatReceive.FLD_TRANSFER_STATUS]+"="+I_DocStatus.DOCUMENT_STATUS_DRAFT+
                            " AND ("+PstMatReceive.fieldNames[PstMatReceive.FLD_RECEIVE_SOURCE]+"="+PstMatReceive.SOURCE_FROM_SUPPLIER+
                            " OR "+PstMatReceive.fieldNames[PstMatReceive.FLD_RECEIVE_SOURCE]+"="+PstMatReceive.SOURCE_FROM_SUPPLIER_PO+")"+
                            ...
                            " OR "+PstMatReceive.fieldNames[PstMatReceive.FLD_RECEIVE_STATUS]+"="+I_DocStatus.DOCUMENT_STATUS_POSTED+")";
        }
...
function change(str, idRcv, idPo, idLocation, idCurrency){
	self.opener.document.forms.frm_retmaterial.<%=FrmMatReturn.fieldNames[FrmMatReturn.FRM_FIELD_INVOICE_SUPPLIER]%>.value = str;
	self.opener.document.forms.frm_retmaterial.<%=FrmMatReturn.fieldNames[FrmMatReturn.FRM_FIELD_RECEIVE_MATERIAL_ID]%>.value = idRcv;
	self.opener.document.forms.frm_retmaterial.<%=FrmMatReturn.fieldNames[FrmMatReturn.FRM_FIELD_PURCHASE_ORDER_ID]%>.value = idPo;
	self.opener.document.forms.frm_retmaterial.<%=FrmMatReturn.fieldNames[FrmMatReturn.FRM_FIELD_LOCATION_ID]%>.value = idLocation;
	self.opener.document.forms.frm_retmaterial.<%=FrmMatReturn.fieldNames[FrmMatReturn.FRM_FIELD_CURRENCY_ID]%>.value = idCurrency;
	self.opener.document.forms.frm_retmaterial.CURRENCY_CODE.value = idCurrency;
	//self.opener.document.forms.frm_retmaterial.all.item("idcommand").style.display = "";
	//self.opener.document.forms.frm_retmaterial.all.item("idcommandback").style.display = "none";
	self.close();
}
