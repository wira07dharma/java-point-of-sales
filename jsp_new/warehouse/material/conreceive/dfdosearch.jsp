...
<%@ page import = "java.util.*,
                   ...
                   com.dimata.posbo.entity.warehouse.PstMatConDispatch,
...
public static final String textListGlobal[][] = {
	{"Daftar Pengiriman Barang","Bulan Pengiriman","Tidak ada data..."},
	{"List of Goods Transfer","Dispatch Month","No available data"}
};

/* this constant used to list text of listHeader */
public static final String textListMatConDispatchHeader[][] = { 
	{"No","Kode","Tanggal","Status","Keterangan"},
	{"No","Code","Date","Status","Remark"}
};

public String drawListMatConDispatch(int language,Vector objectClass,int start,I_DocStatus i_status,int docType,long oidloc) {
	String result = "";
	if(objectClass!=null && objectClass.size()>0) {
		...
		ctrlist.addHeader(textListMatConDispatchHeader[language][0],"3%");	
		ctrlist.addHeader(textListMatConDispatchHeader[language][1],"15%");	
		ctrlist.addHeader(textListMatConDispatchHeader[language][2],"15%");					
		ctrlist.addHeader(textListMatConDispatchHeader[language][3],"15%");		
		ctrlist.addHeader(textListMatConDispatchHeader[language][4],"20%");	
		...
		for(int i=0; i<objectClass.size(); i++) {
			MatConDispatch df = (MatConDispatch)objectClass.get(i);
			start = start + 1;			
            ...
            if(oidloc==df.getDispatchTo()) {
                ...
                lstData.add(rowx);
                lstLinkData.add(df.getOID()+"','"+df.getDispatchCode());
            }
		}
		return ctrlist.draw();
	}
...
long oidloc = FRMQueryString.requestLong(request,"oidloc");
...
String whereClause = " " + PstMatConDispatch.fieldNames[PstMatConDispatch.FLD_LOCATION_ID] + " = " + oidFrom +
		" AND Month(" + PstMatConDispatch.fieldNames[PstMatConDispatch.FLD_DISPATCH_DATE] + " ) = " + monthOfDF +
		" AND Year(" + PstMatConDispatch.fieldNames[PstMatConDispatch.FLD_DISPATCH_DATE] + " ) = " + yearOfDF +
		" AND " + PstMatConDispatch.fieldNames[PstMatConDispatch.FLD_TRANSFER_STATUS] + " = " + i_status.DOCUMENT_STATUS_DRAFT;
		
Vector vect = PstMatConDispatch.list(0,0,whereClause,"");
...
function cmdEdit(dfOid,dfCode) {	
	self.opener.frm_recmaterial.<%=FrmMatReceive.fieldNames[FrmMatReceive.FRM_FIELD_DISPATCH_MATERIAL_ID]%>.value = dfOid;
	self.opener.frm_recmaterial.txt_dfnumber.value = dfCode;
	self.close();		
}
        ...
        <tr> 
          <td> 
            <form name="frmvendorsearch" method="post" action="">
              ...
              <input type="hidden" name="oidloc" value="<%=oidloc%>">
              ...
              <table width="100%" border="0" cellspacing="1" cellpadding="1">
                ...
                <tr> 
                  <td colspan="2"><%=drawListMatConDispatch(SESS_LANGUAGE,vect,start,i_status,docType,oidloc)%></td>
                </tr>
                <tr> 
