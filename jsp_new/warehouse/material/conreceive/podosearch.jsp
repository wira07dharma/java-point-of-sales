	...
	if(objectClass!=null && objectClass.size()>0) {
		...
		for(int i=0; i<objectClass.size(); i++) {
			...
			lstLinkData.add(po.getOID()+"','"+po.getPoCode()+"','"+currencyType.getCode()+"','"+po.getCurrencyId()
							+"', '"+po.getPoStatus()+"', '"+po.getSupplierId()+"', '"+po.getPpn()+"', '"+po.getLocationId());
		}
		return ctrlist.draw();
	}
	else {
...
//System.out.println("monthOfPO : "+monthOfPO+" yearOfPO : "+yearOfPO+" poCode: "+poCode.length());
if(poCode.length() != 0) {
...
function cmdEdit(poOid, poCode, currcode, currOid, poStatus, supplierId, ppn, locationId) {
	if(poStatus != <%=I_DocStatus.DOCUMENT_STATUS_POSTED%>) {
		self.opener.document.forms.frm_recmaterial.<%=FrmMatConReceive.fieldNames[FrmMatConReceive.FRM_FIELD_PURCHASE_ORDER_ID]%>.value = poOid;
		self.opener.document.forms.frm_recmaterial.txt_ponumber.value = poCode;
		self.opener.document.forms.frm_recmaterial.<%=FrmMatConReceive.fieldNames[FrmMatConReceive.FRM_FIELD_CURRENCY_ID]%>.value = currOid;
		self.opener.document.forms.frm_recmaterial.CURRENCY_CODE.value = currOid;
		self.opener.document.forms.frm_recmaterial.<%=FrmMatConReceive.fieldNames[FrmMatConReceive.FRM_FIELD_SUPPLIER_ID]%>.value = supplierId;
		self.opener.document.forms.frm_recmaterial.<%=FrmMatConReceive.fieldNames[FrmMatConReceive.FRM_FIELD_TOTAL_PPN]%>.value = ppn;
		self.opener.document.forms.frm_recmaterial.<%=FrmMatConReceive.fieldNames[FrmMatConReceive.FRM_FIELD_LOCATION_ID]%>.value = locationId;
		self.close();
	}
	else {
