...
<%!    public static final int ADD_TYPE_SEARCH = 0;
    ...
    public static final String textListGlobal[][] = {
        {"Transfer", "Pencarian", "Simpan", "Pilih Semua", "Batal Pilih Semua"},
        {"Dispatch", "Searching", "Save", "Select All", "Deselect All"}
    };

    /* this constant used to list text of listHeader */
    public static final String textListHeader[][] = {
        {"Nomor", "Lokasi Asal", "Lokasi Tujuan", "Tanggal", "Status", "Urut Berdasar", "Semua", "Tanggal Awal", " Tanggal Akhir ", "Lokasi", "Jenis Dokumen", "Waktu"},
        {"Number", "From Location", "To Location", "Date", "Status", "Sort By", "All", "From Date", " to Date ", "Location", "Document Type", "Time"}
    };
    ...
    public static final String textListMaterialHeader[][] = {
        {"No", "Nomor", "Tanggal", "Lokasi Asal", "Lokasi Tujuan", "Status", "Keterangan", "Jenis Dokumen", "Nomor BC"},
        {"No", "Number", "Date", "Dispatch From", "Dispatch To", "Status", "Keterangan", "Document Type", "Customs Number"}
    };

	public static final String responseText[][] = {
		{"Dokumen transfer berhasil dibuat.", "Lokasi asal dan tujuan tidak bisa sama.", "Form Dokumen Transfer Baru"},
		{"The transfer document has been created.", "Origin and destination location must not same.", "New Transfer Document Form"}
	};
    ...
    public String drawList(int language, Vector objectClass, int start, int docType, I_DocStatus i_status) {
        ...
        if (objectClass != null && objectClass.size() > 0) {
            ...
            for (int i = 0; i < objectClass.size(); i++) {
                ...
                rowx.add("<div class=\"text-center\">" + start + "</div>");
                rowx.add("<div class=\"text-center\">" + df.getDispatchCode() + "</div>");
                rowx.add("<div class=\"text-center\">" + df.getJenisDokumen() + "</div>");
                rowx.add("<div class=\"text-center\">" + df.getNomorBeaCukai() + "</div>");
                if(typeOfBusinessDetail == 2){
                    rowx.add(Material.MATERIAL_TYPE_TITLE[newDf.getDispatchItemType()]);
                }
                rowx.add("<div class=\"text-center\">" + Formater.formatDate(df.getDispatchDate(), "dd-MM-yyyy") + "</div>");
				if(dutyFree == 1){
					rowx.add("");
				}
...
select.FRM_FIELD_LOCATION_ID {
                          ...
                          <div class="row">
                            <div class="col-sm-12">
                              ...
                              <div class="col-sm-4">
                                ...
                                <div class="form-group">
                                  ...
                                  <div class="col-sm-8" >
                                          <%
                                            ...
                                            Vector vt_loc_all = PstLocation.listLocationCreateDocument(0, 0, whereClause, "");
                                            val_locationid.add("0");
                                            key_locationid.add(getJspTitle(6, SESS_LANGUAGE, dfCode, true) + " " + getJspTitle(9, SESS_LANGUAGE, dfCode, true));
                                            for (int d = 0; d < vt_loc_all.size(); d++) {
                                              Location loc = (Location) vt_loc_all.get(d);
                                ...
                                <div class="form-group">
                                  ...
                                  <div class="col-sm-8" >
                                          <%
                                            ...
                                            String whereClauseDutyFree = " (" + PstLocation.fieldNames[PstLocation.FLD_TYPE] + " = " + PstLocation.TYPE_DUTY_FREE + ")";

                                            //whereClause += " AND " + PstDataCustom.whereLocReportView(userId, "user_create_document_location");
                                            Vector vt_loc = PstLocation.listLocationCreateDocument(0, 0, whereClauseDutyFree, "");
                                ...
                                <div class="form-group">
                                  <label class="control-label col-sm-4 padd" for="jenisDokumen"><%=getJspTitle(10, SESS_LANGUAGE, dfCode, true)%></label>
                                  <div class="col-sm-8" >
                                    <input id="jenisDokumen" class="form-control" type="text" name="jenisDokumen"  value="PPBTBB" readonly="">
                                  </div>
                                </div>
                              </div>
                              <!-- right side -->             
                                ...
                                <div class="form-group">
                                  ...
                                  <div class="col-sm-8 stt">
                                    <ul class="list-status">
                                            ...
                                            %>
                                            <li>
                                              <input type="checkbox" class="formElemen checks" name="<%=FrmSrcMatDispatch.fieldNames[FrmSrcMatDispatch.FRM_FIELD_STATUS]%>" value="<%=(indexPrStatus)%>">
                                              <%=strPrStatus%>
                                            </li>
                                            <%
                                                }
                                    ...
                                    <a id="UnCheckStatusAllBtn">
                                      <%= textListGlobal[SESS_LANGUAGE][4]%>
                                    </a>
                                  </div>
                                </div> 
                              </div>
                      ...
                      <table width="52%" border="0" cellspacing="0" cellpadding="0">
                        <tr>  
                          ...
                          <td class="command" nowrap width="99%">
                            <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#CreateNewDocModal">
    ...
    <div class="example-modal">
      <div id="CreateNewDocModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          ...
          <div class="modal-content">
            <div class="modal-header">
              ...
              <h4 class="modal-title">
                <%= responseText[SESS_LANGUAGE][2]%>
              </h4>
            </div>
            <div class="modal-body">
              <form id="<%= FrmMatDispatch.FRM_NAME_DISPATCH_MATERIAL%>">
                <table width="100%">
                  <tr>
                    <td style="width:15%">
                    ...
                    <td style="width:5%">:</td>
                    <td style="width:80%">
                      <%=ControlDate.drawDateWithStyle(FrmMatDispatch.fieldNames[FrmMatDispatch.FRM_FIELD_DISPATCH_DATE], new Date(), 1, -5, "formElemen", "")%>
                    </td>
                  </tr>
                  <tr>
                    <td>
                  ...
                  <tr>
                    ...
                    <td>:</td>
                    <td>
                      <%=ControlDate.drawTimeSec(FrmMatDispatch.fieldNames[FrmMatDispatch.FRM_FIELD_DISPATCH_DATE], new Date(), "formElemen")%>
                    </td>
                  </tr>
                  <tr>
                    <td>
                  ...
                  <tr>
                    <td><%= textListHeader[SESS_LANGUAGE][1]%></td>
                    <td>:</td>
                    <td>
                      <%
                        obj_locationid1 = new Vector(1, 1);
                        val_locationid1 = new Vector(1, 1);
                        key_locationid1 = new Vector(1, 1);
                        locWhClause = ""; //PstLocation.fieldNames[PstLocation.FLD_TYPE] + " = " + PstLocation.TYPE_LOCATION_STORE;
                        locOrderBy = String.valueOf(PstLocation.fieldNames[PstLocation.FLD_CODE]);
                        //Vector vt_loc1 = PstLocation.list(0,0,locWhClause,locOrderBy);
                        for (int d = 0; d < vt_loc_all.size(); d++) {
                          Location loc1 = (Location) vt_loc_all.get(d);
                          val_locationid1.add("" + loc1.getOID() + "");
                          key_locationid1.add(loc1.getName());
                        }
                      %>
                      <%=ControlCombo.draw(FrmMatDispatch.fieldNames[FrmMatDispatch.FRM_FIELD_LOCATION_ID], null, "0", val_locationid1, "class=\"" + FrmMatDispatch.fieldNames[FrmMatDispatch.FRM_FIELD_LOCATION_ID] + "\"", key_locationid1, "formElemen")%>
                    </td>
                  </tr>
                  <tr>
                    <td>
                  ...
                  <tr>
                    <td><%= textListHeader[SESS_LANGUAGE][2]%></td>
                    <td>:</td>
                    <td>
                      <%
                        obj_locationid1 = new Vector(1, 1);
                      ...
                      %>
                      <%=ControlCombo.draw(FrmMatDispatch.fieldNames[FrmMatDispatch.FRM_FIELD_DISPATCH_TO], null, "0", val_locationid1, " class=\"" + FrmMatDispatch.fieldNames[FrmMatDispatch.FRM_FIELD_DISPATCH_TO] + "\"", key_locationid1, "formElemen")%>
                    </td>
                  </tr>
                </table>
              </form>
            ...
            <div class="modal-footer">
              <button type="button" id="CreateNewDocBtn" class="btn btn-primary" value="0">
		...
		<script type="text/javascript">
			...
			$(function(){
				...
				$("#CreateNewDocBtn").click(function() {
					...
					var from = $('#<%= FrmMatDispatch.fieldNames[FrmMatDispatch.FRM_FIELD_LOCATION_ID] %>').val();
					console.log("From " + from);
					var destination = $('.<%= FrmMatDispatch.fieldNames[FrmMatDispatch.FRM_FIELD_DISPATCH_TO] %>').val();
					console.log("Destination "+destination);
					
					if(from == destination){
						alert("<%= responseText[SESS_LANGUAGE][1] %>");
					} else {
						var form = $('#<%= FrmMatDispatch.FRM_NAME_DISPATCH_MATERIAL %>');

						console.log(form.serialize());

						$.ajax({
							type: 'POST',
							url: "<%= approot %>/AjaxMatDispatchBill?command=<%= Command.SAVE %>&oid="+oid+"&type=newdocument",
							data: form.serialize(),
							dataType: 'json',
							cache: false,
							success: function (data) {
								alert("<%= responseText[SESS_LANGUAGE][0] %>");
								var start = 0;
								var approval_cmd = "<%= Command.APPROVE %>";
								var cmd = "<%= Command.EDIT %>";
								window.location.replace("df_bc_wh_material_edit.jsp?command="+cmd+"&hidden_dispatch_id="+data.oid+"&approval_command="+approval_cmd);
							}
						});						
					}
			...
			function SelectAll(source, status){
			}
