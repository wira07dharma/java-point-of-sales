<%-- 
    Document   : delivery_order_stockcode_sale
    Created on : Dec 10, 2013, 6:39:46 AM
    Author     : dimata005
--%>
	...
	if(start<0){
	   start = 0;
        ...
        for(int i=0; i< classObj.size(); i++){
            ...
            start = start + 1;
            rowx.add(""+start);
            if(sts==I_DocStatus.DOCUMENT_STATUS_DRAFT || sts==I_DocStatus.DOCUMENT_STATUS_TO_BE_APPROVED){
...
Vector vStock = PstReceiveStockCode.getSerialCodeReceiveBy(material.getOID(),billdetail.getOID());
    if (iCommand == Command.SAVE){
        ...
        ctrlBillDetailCode.requestBarcode(billdetail.getQty(),billdetail.getOID(),request,oidCashBillDetailId);
...
String where = "";
where = PstMaterialStockCode.fieldNames[PstMaterialStockCode.FLD_LOCATION_ID]+"="+billMain.getLocationId()+
        " AND "+PstMaterialStockCode.fieldNames[PstMaterialStockCode.FLD_MATERIAL_ID]+"="+billdetail.getMaterialId()+
...
function cmdBack(){
	...
	document.frm_stockcode.prev_command.value="<%=prevCommand%>";
        ...
        document.frm_stockcode.action="delivery_order_material_edit_sales.jsp";
...
function deleteCode(oidItem){
    ...
    if (isTrue){
        ...
        document.frm_stockcode.prev_command.value="<%=prevCommand%>";
        document.frm_stockcode.action="delivery_order_stockcode_sale.jsp";
        document.frm_stockcode.submit();
    }
}
