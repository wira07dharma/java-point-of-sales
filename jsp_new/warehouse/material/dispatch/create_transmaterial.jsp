...
<%@ page import = "java.util.*,
         com.dimata.posbo.session.masterdata.SessMaterialTransferFromMinumumStock,
...
<%!    public static final String textListGlobal[][] = {
        {"Buat Transfer", "Kembali ke Minimum Stock", "Daftar Transfer"},
        {"Create Transfer", "Back to Minimum Stock", "Transfer List"}
    };

    /* this constant used to list text of listHeader */
    public static final String textListMaterialHeader[][] = {
        {"No", "Kode Barang", "Nama Barang", "Qty Transfer", "Location Asal", "Daftar Barang Order", "Lokasi Tujuan"},
        {"No", "Code", "Product Name", "Qty Transfer", "Source Location", "Product Order List", "Destiny Location"}
    };

    public String drawList(Vector list, Vector listQty, int language, long location_oid_select, long supplier_id_select, long lokasi_source_id) {
        ...
        ctrlist.addHeader("<div align=\"center\">" + textListMaterialHeader[language][0] + "</div>", "4%");
        ctrlist.addHeader("<div align=\"center\">" + textListMaterialHeader[language][1] + "</div>", "8%");
        ctrlist.addHeader("<div align=\"center\">" + textListMaterialHeader[language][2] + "</div>", "25%");
        ctrlist.addHeader("<div align=\"center\">" + textListMaterialHeader[language][3] + "</div>", "8%");
        ctrlist.addHeader("<div align=\"center\">" + textListMaterialHeader[language][6] + "</div>", "8%");
        ctrlist.addHeader("<div align=\"center\">" + textListMaterialHeader[language][4] + "</div>", "25%");
        ...
        if (location_oid_select != 0) {
            ...
            whereSource = PstLocation.fieldNames[PstLocation.FLD_LOCATION_ID] + " NOT IN ('" + location_oid_select + "')";
        }
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            rowx.add(ControlCombo.draw("supplier_order_name", null, "" + lokasi_source_id, val_lokasi_source, key_lokasi_source, "", "formElemen"));
...
<%    
    ...
    long lokasi_source_id = FRMQueryString.requestLong(request, "lokasi_source_id");
    ...
    Vector list = SessMaterialTransferFromMinumumStock.getListItemForTransfer(vectQty);
    if (finish == 1) {
        vectQty = new Vector();
        if (txtqtypo.length > 0) {
            for (int k = 0; k < txtqtypo.length; k++) {
            ...
            if (vectQty.size() > 0) {
                SessMaterialTransferFromMinumumStock.autoInsertTransfer(vectQty);
            } else {
                if (txtqtypo.length > 0) {
                    for (int k = 0; k < txtqtypo.length; k++) {
                        long oidMaterial = Long.parseLong(txtqtypo[k]);
    ...
    <head>
        ...
        <script language="JavaScript">
            function cmdSave()
            {
                ...
                document.frm_newpo.action = "create_transmaterial.jsp";
            ...
            function cmdFinish() {
                ...
                //}
                //else {
                //alert("Supplier tidak ada!");
                //}
            }

            function cmdBack() {
                document.frm_newpo.command.value = "<%=Command.BACK%>";
                document.frm_newpo.action = "../../../warehouse/material/report/reportstockmin_transfer_list.jsp";// "srcpomaterial.jsp";
                document.frm_newpo.submit();
            }

            function cmdBackPo() {
                document.frm_newpo.command.value = "<%=Command.BACK%>";
                document.frm_newpo.action = "../../../warehouse/material/report/reportstockmin_transfer_list.jsp";
                document.frm_newpo.submit();
            }
                        ...
                        <tr> 
                            <td><!-- #BeginEditable "content" -->
                                ...
                                <form name="frm_newpo" method="post" action="">
                                    ...
                                    <input type="hidden" name="lokasi_source_id"  value="<%=lokasi_source_id%>">
                                    <table width="100%" border="0" cellspacing="1" cellpadding="1">
                                        ...
                                        %>
                                        <tr>
                                            <td colspan="2"><%= drawList(list, vectQty, SESS_LANGUAGE, location_id_select, supplier_id_select, lokasi_source_id)%></td>
                                        </tr>
                                        <tr>
                                            <td span class="command">
                                                ...
                                                %>
                                                <%//=ctrlLine.drawImageListLimit(iCommand ,vectSize,start,recordToGet)%></td>
                                        </tr>
                                        <%
                                                } catch (Exception exc) {
