<%-- 
    Document   : df_stockcode
    Created on : Nov 18, 2013, 5:56:38 PM
    Author     : dimata005
--%>
                                ...
                                double max ,long soItemId,int start, Vector listObj, int sts, double valueHpp,boolean privShowQtyPrice)
{
	...
	ctrlist.addHeader(textListOrderItem[language][1] +" | <a href='javascript:deleteAll()'>Clear All</a>","25%");
        if(privShowQtyPrice){
	...
	if(start<0){
	   start = 0;
        ...
        for(int i=0; i< max; i++){
            ...
            rowx.add("<div align=\"left\"><input name=\""+frmObject.fieldNames[FrmDispatchStockCode.FRM_FIELD_STOCK_CODE] +"_"+i+"_oid\" type=\"hidden\" value=\""+dispatchStockCode.getOID()+"\"><input type=\"text\" name=\""+frmObject.fieldNames[FrmDispatchStockCode.FRM_FIELD_STOCK_CODE] +"_"+i+"\" value=\""+strCode+"\" onkeyup=\"if (event.keyCode == 13) { showData(this.value); return false; }\" class=\"formElemen\"></div>");
...
try{
	matDispatchItem = PstMatDispatchItem.fetchExc(oidDispatchMaterialId);
        ...
        oidDispatchId = matDispatch.getOID();
...
if (iCommand == Command.GET){
        ...
        //dyas 20131202
        //tambah variabel oidDispatchId
        ctrDispatchStockCode.generateBarcode(matDispatchItem.getQty(),matDispatchItem.getOID(),request,oidDispatchId);
        iCommand = Command.ADD;
    } else if (iCommand == Command.SAVE){

        //dyas 20131202
        //tambah variabel oidDispatchId
        ctrDispatchStockCode.requestBarcode(matDispatchItem.getQty(),matDispatchItem.getOID(),request,oidDispatchId);
        iCommand = Command.ADD;
    }else if(iCommand == Command.DELETE){
        ctrDispatchStockCode.action(iCommand,oidSourceStockCodeId);
        iCommand = Command.ADD;
    } else if (iCommand == Command.RESET){
		...
		if (listDispatchStockCode.size()>0){
			for (int i = 0; i < listDispatchStockCode.size();i++){
				DispatchStockCode stockCode = (DispatchStockCode) listDispatchStockCode.get(i);
				ctrDispatchStockCode.action(Command.DELETE,stockCode.getOID());
			}
		}
		iCommand = Command.ADD;
	}
...
//list stock yang bisa di transfer
String where = "";
...
function cmdBack(){
    ...
    <%if(type==0){%>
	document.frm_stockcode.action="df_stock_wh_material_edit.jsp";
    <%}else if(type==1){%>
        document.frm_stockcode.action="df_unit_wh_material_item.jsp";
    <%}%>
...
function deleteCode(oidItem){
    ...
    //if (isTrue){
        ...
        document.frm_stockcode.prev_command.value="<%=prevCommand%>";
        document.frm_stockcode.action="df_stockcode.jsp";
        document.frm_stockcode.submit();
    //}
}
...
function deleteAll(){
	document.frm_stockcode.command.value="<%=Command.RESET%>";
	document.frm_stockcode.prev_command.value="<%=prevCommand%>";
	document.frm_stockcode.action="df_stockcode.jsp";
	document.frm_stockcode.submit();
}
...
function checkAjax(value){
    $.ajax({
    url : "<%=approot%>/servlet/com.dimata.posbo.ajax.CheckStockCode?<%=FrmDispatchStockCode.fieldNames[FrmDispatchStockCode.FRM_FIELD_STOCK_CODE]%>="+value+"&<%=FrmMatDispatch.fieldNames[FrmMatDispatch.FRM_FIELD_DISPATCH_MATERIAL_ID]%>=<%=oidDispatchId%>&<%=CheckStockCode.CHECK_STOCK_CODE_NAME%>=<%=CheckStockCode.CHECK_STOCK_CODE_DISPATCH%>",
    type : "POST",
    async : false,
...
<!--                    dyas 20131202
              ...
              <table width="100%"  border="0" cellspacing="1" cellpadding="1">
                  ...
                  <td width="2%"><table width="100%"  border="0" cellspacing="1" cellpadding="1">
					...
					<tr>
						 <td><%
								...
								if(matDispatch.getDispatchStatus() !=I_DocStatus.DOCUMENT_STATUS_DRAFT){
					...
					<tr>
						<td class="formElemen" colspan="2">Transfer From
							<input type="text" size="20" name="GENERATE_FROM" onkeyup="javascript:keyDownCheck(event)">
							<a href="javascript:cmdGenerate()" class="btn btn-primary"><i class="fa fa-arrow-left"></i> Transfer</a>
						</td>	
					</tr>
                  </table></td>
                </tr>
                <tr>
                  <td><%= drawList(SESS_LANGUAGE,iCommand,frmDispatchStockCode, sourceStockCode ,listDispatchStockCode,max,oidDispatchMaterialId,start,vStock,matDispatch.getDispatchStatus(),valueHpp,privShowQtyPrice)%></td>
                </tr>
                <tr>
                  <td>&nbsp;</td>
                </tr>
                ...
                <script type="text/javascript">
                        document.all.<%=FrmDispatchStockCode.fieldNames[FrmDispatchStockCode.FRM_FIELD_STOCK_CODE]%>_0.focus();
                </script>
          </td>
        </tr>
      </table>
