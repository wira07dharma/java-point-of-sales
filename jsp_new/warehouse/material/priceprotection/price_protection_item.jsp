<%-- 
    Document   : price_protection_item.jsp
    Created on : Dec 30, 2014, 3:13:52 PM
    Author     : dimata005
--%>
...
*/
public Vector drawListDfItem(int language,int iCommand,FrmPriceProtectionItem frmObject,PriceProtectionItem objEntity,Vector objectClass,long dfItemId,long oidPriceProtection, String approot)
{
        ...
        for(int i=0; i<objectClass.size(); i++)
	{
			...
			"\"><input type=\"text\" size=\"13\" onKeyDown=\"javascript:keyDownCheck(event)\" name=\"matCode\" value=\""+mat.getSku()+"\" class=\"formElemen\">&nbsp;<a href=\"javascript:cmdCheck()\">CHK</a>");
                        ...
                        rowx.add("<div align=\"center\"> <input type=\"text\" size=\"20\" name=\""+frmObject.fieldNames[frmObject.FRM_FLD_QTY_ON_HAND]+"\" value=\""+priceProtectionItem.getStockOnHand()+"\" class=\"formElemen\" onKeyUp=\"javascript:cntTotal(this, event)\" align=\"center\"></div>");
                        rowx.add("<div align=\"center\"> <input type=\"text\" size=\"20\" name=\""+frmObject.fieldNames[frmObject.FRM_FLD_AMOUNT]+"\" value=\""+priceProtectionItem.getAmount()+"\" class=\"formElemen\" onKeyUp=\"javascript:cntTotal(this, event)\" align=\"center\"></div>");
                        rowx.add("<div align=\"center\"> <input type=\"text\" size=\"20\" name=\""+frmObject.fieldNames[frmObject.FRM_FLD_TOTAL_AMOUNT]+"\" value=\""+priceProtectionItem.getTotalAmount()+"\" class=\"formElemen\" onKeyUp=\"javascript:cntTotal(this, event)\" align=\"center\"></div>");
                }else{
                    rowx.add(""+start+"");
	...
	if(iCommand==Command.ADD || (iCommand==Command.SAVE && frmObject.errorSize()>0)){
		...
		rowx.add("<input type=\"hidden\" name=\""+frmObject.fieldNames[frmObject.FRM_FLD_MATERIAL_ID]+"\" value=\"\"> "
                ...
                rowx.add("<div align=\"center\"> <input type=\"text\" size=\"20\" name=\""+frmObject.fieldNames[frmObject.FRM_FLD_QTY_ON_HAND]+"\" value=\""+""+"\" class=\"formElemen\" onKeyUp=\"javascript:cntTotal(this, event)\" align=\"center\"></div>");
                rowx.add("<div align=\"center\"> <input type=\"text\" size=\"20\" name=\""+frmObject.fieldNames[frmObject.FRM_FLD_AMOUNT]+"\" value=\""+""+"\" class=\"formElemen\" onKeyUp=\"javascript:cntTotal(this, event)\"  align=\"center\"></div>");
                rowx.add("<div align=\"center\"> <input type=\"text\" size=\"20\" name=\""+frmObject.fieldNames[frmObject.FRM_FLD_TOTAL_AMOUNT]+"\" value=\""+""+"\" class=\"formElemen\" onKeyUp=\"javascript:cntTotal(this, event)\" align=\"center\"></div>");
...
ctrlPriceProtectionItem.setLanguage(SESS_LANGUAGE);


//by dyas - 20131126
//tambah variabel userName dan userId
iErrCode = ctrlPriceProtectionItem.action(iCommand,oidPriceProtectionItem,oidPriceProtection, userName, userId);
...
msgString = ctrlPriceProtectionItem.getMessage();
...
{
	start = ctrlPriceProtectionItem.actionList(iCommand,start,vectSizeItem,recordToGetItem);
}

Vector listPriceProtectionItem = PstPriceProtectionItem.listInnerJoinMaterial(start,recordToGetItem,whereClousex,PstPriceProtectionItem.fieldNames[PstPriceProtectionItem.FLD_POS_PRICE_PROTECTION_ITEM_ID]);

if(listPriceProtectionItem.size()<1 && start>0)
{
     ...
     listPriceProtectionItem = PstPriceProtectionItem.list(start,recordToGetItem,"","");
}
...
function cntTotal(element, evt){
   ...
   var qty = cleanNumberFloat(document.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_QTY_ON_HAND]%>.value,guiDigitGroup,guiDecimalSymbol);
   //alert("asas "+qty);
   if(qty>0.0000){
      //alert("perhitunga");
      var amount = cleanNumberFloat(document.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_AMOUNT]%>.value,guiDigitGroup,guiDecimalSymbol);
      var total = amount*qty;
      //alert("jumlah " +total);
      document.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_TOTAL_AMOUNT]%>.value=total;
      //return;
    }else{
        
        alert("item ini tidak bisa mendapatkan PP, stok 0");
        document.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_AMOUNT]%>.value=0;
        document.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_TOTAL_AMOUNT]%>.value=0;
...
function changeFocus(element){
    //alert(element.name);
    if(element.name == "<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_QTY_ON_HAND]%>") {

        document.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_AMOUNT]%>.focus();

    }

    else if(element.name == "<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_AMOUNT]%>") {
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">  
        ...
        <tr> 
	      ...
	      <input type="hidden" name="hidden_priceprotection_id" value="<%=oidPriceProtection%>">
              ...
              <input type="hidden" name="<%=FrmPriceProtection.fieldNames[FrmPriceProtection.FRM_FLD_TOTAL_AMOUNT]%>" value="<%=sumTotal%>">
                  ...
                  <td colspan="3" class="title">
                    <table width="100%" border="0" cellpadding="1">
                      ...
                      <tr>
                        ...
                        //add by fitra
                        if(listPriceProtectionItem.size()>0){
                                    val_status.add(String.valueOf(I_DocStatus.DOCUMENT_STATUS_TO_BE_APPROVED));
                                    key_status.add(I_DocStatus.fieldDocumentStatus[I_DocStatus.DOCUMENT_STATUS_TO_BE_APPROVED]);
                        }
                        if(listPriceProtectionItem.size()>0 && privApproval==true && locationAssign==true){
                          ...
                          <table width="100%" border="0" cellspacing="0" cellpadding="0" >
                            <tr align="left" valign="top">
                              ...
                              <td height="22" valign="middle" colspan="3"> <%
                                                                ...
                                                                Vector list = drawListDfItem(SESS_LANGUAGE,iCommand,frmPriceProtectionItem,priceProtectionItem,listPriceProtectionItem,oidPriceProtectionItem,oidPriceProtection,approot);
