<%-- 
    Document   : priceprotectiondosearch
    Created on : Dec 30, 2014, 7:21:47 PM
    Author     : dimata005
--%>
...
public String drawListMaterial(int language,Vector objectClass, Vector vectDf, int start,int tranUsedPriceHpp, int enableStokFisik)
{
	String result = "";
           ...
           // long oid = getOidDfItem(vectDf,material.getOID());
            ...
            /*if(tranUsedPriceHpp==0){
                    lstLinkData.add(material.getOID()+"','"+material.getSku()+"','"+name+"','"+unit.getCode()+
                            "','"+FRMHandler.userFormatStringDecimal(material.getAveragePrice())+"','"+material.getDefaultPrice()+"','"+material.getDefaultStockUnitId()+"','"+materialStock.getQty()+"','"+enableStokFisik+"','0");
            }else{
                    ...
                    lstLinkData.add(material.getOID()+"','"+material.getSku()+"','"+name+"','"+unit.getCode()+
                            "','"+FRMHandler.userFormatStringDecimal(sellPrice)+"','"+material.getDefaultPrice()+"','"+material.getDefaultStockUnitId()+"','"+materialStock.getQty()+"','"+enableStokFisik+"','0");
            }*/			
           lstLinkData.add(material.getOID()+"','"+material.getSku()+"','"+name+"','"+materialStock.getQty()+"");
        }
		return ctrlist.draw();
	}else{
...
if(vectSize == 1 && automaticSelected==0) {
    ...
    <script language="JavaScript">
        ...
        self.opener.document.forms.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_QTY_ON_HAND]%>.value="<%=materialStock.getQty()%>";
        self.opener.document.forms.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_MATERIAL_ID]%>.value ="<%=material.getOID()%>";
        self.opener.document.forms.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_AMOUNT]%>.focus();
        self.close();
    </script>
    <%
}
...
function cmdEdit(matOid,matCode,matItem, qty){
        ...
        self.opener.document.forms.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_QTY_ON_HAND]%>.value=qty;
        self.opener.document.forms.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_MATERIAL_ID]%>.value = matOid;
        self.opener.document.forms.frm_matdispatch.<%=FrmPriceProtectionItem.fieldNames[FrmPriceProtectionItem.FRM_FLD_AMOUNT]%>.focus();
	self.close();		
}
...
function cmdListFirst()
{
	...
	document.frmvendorsearch.action="priceprotectiondosearch.jsp";
        ...
        <tr> 
	      ...
	      <input type="hidden" name="hidden_dispatch_id" value="<%=oidDispatchMaterial%>">
              ...
              <input type="hidden" name="trap" value="">
                ...
                <tr> 
                  <td colspan="2"><%=drawListMaterial(SESS_LANGUAGE,vect,vectCos,start,tranUsedPriceHpp,enableStokFisik)%></td>
                </tr>
                <tr> 
