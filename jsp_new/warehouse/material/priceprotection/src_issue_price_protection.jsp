<%-- 
    Document   : src_issue_price_protection
    Created on : Jan 2, 2015, 8:29:21 PM
    Author     : dimata005
--%>
...
public static final String textListGlobal[][] = {
	{"Price Protection","Issue Price Protection","Pencarian","Daftar","Edit","","Tanpa PO","Tidak ada data penerimaan barang"},
	{"Price Protection","Issue Price Protection","Search","List","Edit","","Without PO","Receiving goods not found"}
};
...
public static final String textListMaterialHeader[][] = {
	{"No","Kode","Tanggal","Status","Amount","Tanggal Update","Update","Keterangan"},
	{"No","Code","Date","Status","Amount","Update Date","Update","Description"}
};
...
public String drawList(int language,Vector objectClass,int start,int docType,I_DocStatus i_status) {
	...
	if(objectClass!=null && objectClass.size()>0) {
		...
		ctrlist.addHeader(textListMaterialHeader[language][4],"12%");
		ctrlist.addHeader(textListMaterialHeader[language][5],"15%");
                ctrlist.addHeader(textListMaterialHeader[language][6],"14%");
		...
		for(int i=0; i<objectClass.size(); i++) {
			...
			rowx.add(i_status.getDocStatusName(docType,priceProtection.getStatus()));//3
                        ...
                        if(priceProtection.getStatus()==5||priceProtection.getStatus()==7){
                            rowx.add("<div align=\"left\">"+Formater.formatDate(priceProtection.getDateApproved(), "dd-MM-yyyy")+"</div>");
                            rowx.add("<a href=\"javascript:cmdEdit('"+String.valueOf(priceProtection.getOID())+"')\">Distribusi PP</a>");
                        }else{
                            rowx.add("<div align=\"left\">"+ControlDate.drawDate("date_update_"+priceProtection.getOID(), new Date(),"formElemen", 0, 0)+"</div>");
...
if(iCommand==Command.UPDATE){
         ...
         int x = SessPriceProtection.issuePriceProtection(request, records);
         records = SessPriceProtection.searchPriceProtection(srcMatReceive,start,recordToGet,whereClausex);        
}
...
function cmdSearch(){
	document.frm_src_issue_price_protection.command.value="<%=Command.LIST%>";
	document.frm_src_issue_price_protection.action="src_issue_price_protection.jsp";
	document.frm_src_issue_price_protection.submit();
}

function cmdAddPO(){
	document.frm_src_issue_price_protection.command.value="<%=Command.ADD%>";
	document.frm_src_issue_price_protection.approval_command.value="<%=Command.SAVE%>";	
	document.frm_src_issue_price_protection.add_type.value="<%=ADD_TYPE_SEARCH%>";				
	document.frm_src_issue_price_protection.action="price_protection_edit.jsp";
	document.frm_src_issue_price_protection.submit();
}

function cmdUpdate(){
	document.frm_src_issue_price_protection.command.value="<%=Command.UPDATE%>";				
	document.frm_src_issue_price_protection.action="src_issue_price_protection.jsp";
	document.frm_src_issue_price_protection.submit();
}
...
function cmdAdd(){
	...
	document.frm_src_issue_price_protection.approval_command.value="<%=Command.SAVE%>";	
	document.frm_src_issue_price_protection.add_type.value="<%=ADD_TYPE_SEARCH%>";				
	document.frm_src_issue_price_protection.action="receive_wh_supp_material_edit.jsp";
	if(compareDateForAdd()==true)
	document.frm_src_issue_price_protection.submit();
}

function cmdEdit(idRcv){
	document.frm_src_issue_price_protection.hidden_priceprotection_id.value=idRcv;
	document.frm_src_issue_price_protection.start.value=0;
	document.frm_src_issue_price_protection.approval_command.value="<%=Command.APPROVE%>";					
	document.frm_src_issue_price_protection.command.value="<%=Command.ADD%>";
	document.frm_src_issue_price_protection.action="distribution_price_protection.jsp";
	document.frm_src_issue_price_protection.submit();
}

function cmdListFirst(){
	document.frm_src_issue_price_protection.command.value="<%=Command.FIRST%>";
	document.frm_src_issue_price_protection.action="src_issue_price_protection.jsp";
	document.frm_src_issue_price_protection.submit();
}

function cmdListPrev(){
	document.frm_src_issue_price_protection.command.value="<%=Command.PREV%>";
	document.frm_src_issue_price_protection.action="src_issue_price_protection.jsp";
	document.frm_src_issue_price_protection.submit();
}

function cmdListNext(){
	document.frm_src_issue_price_protection.command.value="<%=Command.NEXT%>";
	document.frm_src_issue_price_protection.action="src_issue_price_protection.jsp";
	document.frm_src_issue_price_protection.submit();
}

function cmdListLast(){
	document.frm_src_issue_price_protection.command.value="<%=Command.LAST%>";
	document.frm_src_issue_price_protection.action="src_issue_price_protection.jsp";
	document.frm_src_issue_price_protection.submit();
}

function cmdBack(){
	document.frm_src_issue_price_protection.command.value="<%=Command.BACK%>";
	document.frm_src_issue_price_protection.action="src_issue_price_protection.jsp";
	document.frm_src_issue_price_protection.submit();
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" height="100%" bgcolor="#FCFDEC" >
  <form name="frm_src_issue_price_protection" method="post" action="">
  ...
  <tr> 
    <td valign="top" align="left"> 
      ...
      <tr align="left" valign="top">
         ...
         <%if(iCommand==Command.LIST || iCommand==Command.FIRST || iCommand==Command.PREV || iCommand==Command.NEXT || iCommand == Command.LAST || iCommand == Command.UPDATE){%>	 
        <tr align="left" valign="top"> 
         ...
         <tr align="left" valign="top">
                  <td height="18" valign="top" colspan="3"> 
                    <table width="50%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                             <% if(records.size()!=0){%>
                                <td nowrap width="4%"><a href="javascript:cmdUpdate()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image100','','<%=approot%>/images/BtnNewOn.jpg',1)"><img name="Image100" border="0" src="<%=approot%>/images/BtnNew.jpg" width="24" height="24" alt="<%=ctrLine.getCommand(SESS_LANGUAGE,textListGlobal[SESS_LANGUAGE][0]+" "+textListGlobal[SESS_LANGUAGE][5],ctrLine.CMD_ADD,true)%>"></a></td>
                                <td class="command" nowrap width="31%"><a href="javascript:cmdUpdate()"><%=ctrLine.getCommand(SESS_LANGUAGE,textListGlobal[SESS_LANGUAGE][1]+" "+textListGlobal[SESS_LANGUAGE][5],ctrLine.CMD_SAVE,true)%></a></td>
                             <%}%> 
                      </tr>
                    </table>
                  </td>
