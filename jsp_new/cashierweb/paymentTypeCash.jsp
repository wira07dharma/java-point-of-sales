...
<%@page import="com.dimata.pos.form.payment.CtrlCashPayment"%>
...
double amountTotalPaid =  FRMQueryString.requestLong(request,FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_AMOUNT]);
...
long cashCashierId=FRMQueryString.requestLong(request, FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_CASHCASHIER_ID]);//"504404544260781250";
long locationId=0;//FRMQueryString.requestLong(request, FrmCashPayment.fieldNames[FrmCashPayment.FRM_field_]);//"504404544214899588";
long shiftId=0;//FRMQueryString.requestLong(request, FrmCashPayment.fieldNames[FrmCashPayment.FRM_]);//"504404535341007539";
...
iErrCode = ctrlCashPayment.action(iCommand, oidPaymentSelected, oidInvoiceSelected,0);
FrmCashPayment frmCashPayment = ctrlCashPayment.getForm();
CashPayments1 ap = ctrlCashPayment.getCashPayment();
errMsg = ctrlCashPayment.getMessage();

PaymentInfo paymentInfo = ctrlCashPayment.getPaymentInfo();
...
if(iCommand==Command.UPDATE){
    int xxx = ctrlCashPayment.requestPayment(request, customerId,amountTotalPaid,cashCashierId,shiftId,locationId,userId,oidBillMain);
}
...
String whereClauseBillMain = PstBillMain.fieldNames[PstBillMain.FLD_DOC_TYPE] + " = '0' AND " + 
                             PstBillMain.fieldNames[PstBillMain.FLD_TRANSCATION_TYPE] + " = '0' AND " +
                             PstBillMain.fieldNames[PstBillMain.FLD_TRANSACTION_STATUS] + " = '1' AND " +
...
records = PstBillMain.listInvoiceToPayment(0,0, whereClauseBillMain, orderClauseBillMain);//SessAccPayable.getListAP(srcAccPayable, start, recordToGet,oidReceive );
...
function changePaymentSystem() {
    ...
    var paymentId = document.frap.<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_TYPE]%>.value;
    <%
...
function changeDailyRate() {
	var currency = document.frap.<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_CURR_ID]%>.value;
	...
	switch(currency) {
		...
		%>
			case "<%=currType.getOID()%>":
				...
				%>
				document.frap.<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_RATE]%>.value = '<%=FRMHandler.userFormatStringDecimal(dr.getSellingRate())%>';
				break;
		<%
		}
		%>
...
function calculate() {
	var rate = cleanNumberFloat(document.frap.<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_RATE]%>.value, guiDigitGroup, guiDecimalSymbol);
	var amount = cleanNumberFloat(document.frap.<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_AMOUNT]%>.value, guiDigitGroup, guiDecimalSymbol);
...
function cmdListFirst(){
	...
	document.frap.action="paymentTypeCash.jsp";
	document.frap.submit();
}
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td><!-- #BeginEditable "content" -->
            ...
            <%--
            <input type="hidden" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_LOCATION_ID]%>" value="<%=locationId%>">
            <input type="hidden" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_SHIFT_ID]%>" value="<%=shiftId%>">
            <!--input type="hidden" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_SALES_CODE]%>" value="<//%=salesCode%>"-->
            --%>
            <table width="100%" border="0" cellspacing="1" cellpadding="1">
                            ...
                            <td width="100%">
                              <table width="100%"  border="0" class="listgen" cellspacing="1" cellpadding="1">
                                ...
                                for(int i=0; i<records.size(); i++) {
                                        ...
                                        double total = billMain.getAmount()+billMain.getServiceValue()-billMain.getDiscount();//Double.parseDouble((String)temp.get(5));
                                        double tax = billMain.getTaxValue();
                                ...
                                <tr class="<%=recordCss%>">
                                        ...
                                        <td align="center"><input type="checkbox" name="invoice_<%=billMain.getOID()%>" value="1" onclick="checkPaid(this,'<%=apSaldo%>')" check >Paid</td>
                                </tr>
                                    ...
                                    <table width="100%" border="0"  class="listgen" cellspacing="1" cellpadding="1">
                                      <tr class="listgentitle" align="center">
                                        ...
                                        <td width="20%"><strong><%=textListHeaderDetail[SESS_LANGUAGE][6]%></strong></td>
                                          ...
                                          <tr class="listgensell">
                                                <td>
                                                   <%=ControlDate.drawDate(FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_DATETIME], arPayment.getPayDate(), "formElemen", 1,-5) %>
                                                   <input type="hidden" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_TYPE]%>" value="<%=oidPayment%>">
                                                   <input type="hidden" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_CASH_BILL_MAIN_ID]%>" value="<%=oidInvoice%>">
                                                </td>
                                                <td>
                                                   <%=ControlCombo.draw(FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_TYPE], null, ""+paymentSystem.getOID(), listPaySysVal, listPaySysKey, "onchange=\"changePaymentSystem();\"", "formElemen")%>
                                                   <input type="hidden" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_CASH_BILL_MAIN_ID]%>" value="<%=oidInvoice%>">
                                                   <input type="hidden" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_TYPE]%>" value="<%=arPayment. getCreditPaymentMainId() %>">
                                                </td>
                                                <td><%=ControlCombo.draw(FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_CURR_ID], null, ""+currencyType.getOID(), listCurrTypeVal, listCurrTypeKey, "onchange=\"changeDailyRate();\"", "formElemen")%></td>
                                                <td align="right"><input type="text" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_RATE]%>"  value="<%=FRMHandler.userFormatStringDecimal(arPayment.getRate())%>" class="formElemen" size="14"></td>
                                                <td align="right"><input type="text" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_AMOUNT]%>"  value="<%=FRMHandler.userFormatStringDecimal(arPayment.getTotalPay())%>" class="formElemen" size="15" onKeyUp="javascript:calculate()"></td>
               ...
               <tr>
                  <td valign="middle" colspan="2">
                    <table border="0" width="100%" cellspacing="0" cellpadding="0">
                      <tr>
                        <td height="8" class="comment">Payment</td>
                      </tr>
                      <tr>
                        <td width="100%">
                          <table width="100%"  border="0" class="listgen" cellspacing="1" cellpadding="1">
                              <% if(iCommand == Command.ADDALL) { %>
                                 ...
                                 <tr class="listgensell">
                                        <td><%=ControlDate.drawDate(FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_DATETIME], new Date(), "formElemen", 1,-5) %>
                                        </td>
                                        <td><%=ControlCombo.draw(FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_TYPE], null, "", listPaySysVal, listPaySysKey, "onchange=\"changePaymentSystem();\"", "formElemen")%></td>
                                        <td><%=ControlCombo.draw(FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_CURR_ID], null, "", listCurrTypeVal, listCurrTypeKey, "onchange=\"changeDailyRate();\"", "formElemen")%></td>
                                        <td align="right"><input type="text" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_RATE]%>"  value="" class="formElemen" size="14"></td>
                                        <td align="right"><input type="text" name="<%=FrmCashPayment.fieldNames[FrmCashPayment.FRM_FIELD_PAY_AMOUNT]%>"  value="" class="formElemen" size="15" onKeyUp="javascript:calculate()"></td>
                                    ...
                                    <% if(iCommand != Command.ADDALL) { %>
                                      ...
                                      <tr>
                                        <td colspan="4">
                                                    ...
                                                    if(iCommand==Command.SAVE && frmCashPayment.errorSize()==0 || iCommand == Command.ADD){
                                                            ctrLineAllPayment.setSaveCaption("");
                                                            ctrLineAllPayment.setBackCaption("");
                                                    }
