 ...
 */
    ...
    {
        {"Daftar ","Harus diisi", "Cetak "},
        {"Registration Member","required", "Print "}
    };

	public static final String textListHeader[][] =
    {//     0                1      2             3     4              5         6        7           8 
        {"Tgl Registrasi","Kode"," Name","E-mail","Username","Password","Negara","Provinsi"," Kecamatan", //8
...
if(iCommand==Command.LIST||iCommand == Command.FIRST || iCommand == Command.PREV ||
  iCommand == Command.NEXT || iCommand == Command.LAST || iCommand == Command.BACK){
	try{
		srcMemberReg = (SrcMemberReg)session.getValue(SessMemberReg.SESSION_SRC_GUIDE);
	}catch(Exception e){
		srcMemberReg = new SrcMemberReg();
	}

	if(session.getValue(SessMemberReg.SESSION_SRC_GUIDE)==null){
		srcMemberReg = new SrcMemberReg();
	}

	session.putValue(SessMemberReg.SESSION_SRC_GUIDE,srcMemberReg);
}
...
int vectSize = SessMemberReg.countGuide(0,recordToGet,whContacts,wheress);
...
if ((iCommand == Command.FIRST || iCommand == Command.PREV)
            ...
            //listContact = PstContact.list(start, recordToGet, whereClause, orderClause);




listContact = SessMemberReg.searchGuide(srcMemberReg,start,recordToGet);
...
ctrlContact.action(iCommand , oidContact, 0,0);

if((iCommand==Command.FIRST)||(iCommand==Command.NEXT)||(iCommand==Command.PREV)||(iCommand==Command.LAST)||(iCommand==Command.LIST)) {
	
    
    start = ctrlContact.actionList(iCommand,  vectSize,   start,  recordToGet);
}
*/
...
String whContact = "CLS."+ PstContactClass.fieldNames[PstContactClass.FLD_CLASS_TYPE]+ " = " +contactClassType;
 ...
 listContact = SessMemberReg.searchContact(start,recordToGet,whContact ,wheres);*/
...
function cmdAdd(){
	...
	document.frmmemberreg.action="guide_edit.jsp";
...
function cmdAsk(oidContact){
	...
	document.frmmemberreg.prev_command.value="<%=prevCommand%>";
	document.frmmemberreg.action="guidereg_list.jsp";
	document.frmmemberreg.submit();
}
...
function cmdBack(){
	...
	document.frmmemberreg.action="srcguide.jsp";
	document.frmmemberreg.submit();
	}
                        ...
                        <td height="22" valign="middle" colspan="3">
                          <table width="60%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                              ...
                              <td width="45%" nowrap class="command"><a class="btn btn-lg btn-primary" href="javascript:cmdAdd()"><i class="fa fa-plus-circle"></i> <%=ctrLine.getCommand(SESS_LANGUAGE,textListTitle[SESS_LANGUAGE][0],ctrLine.CMD_ADD,true)%></a></td>
                              <%}}%>
                              ...
                              <td width="45%" nowrap class="command"><a class="btn btn-lg btn-primary" href="javascript:cmdBack()"><i class="fa fa-arrow-left"></i> <%=ctrLine.getCommand(SESS_LANGUAGE,textListTitle[SESS_LANGUAGE][0],ctrLine.CMD_BACK_SEARCH,true)%></a></td>
                              ...
                              <td width="30%" nowrap>&nbsp; <a class="btn btn-lg btn-primary" href="javascript:printForm()" class="command" ><i class="fa fa-print"></i> <%=textListTitle[SESS_LANGUAGE][2]+" "+textListTitle[SESS_LANGUAGE][0]%></a></td>
                            </tr>
                          </table>
                        </td>
                      </tr>
