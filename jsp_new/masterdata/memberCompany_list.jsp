 ...
 */
    ...
    {
        {"Daftar Perusahaan","Harus diisi", "Cetak "},
        {"Daftar Company","required", "Print "}
    };

	public static final String textListHeader[][] =
    {//     0                1      2             3     4              5         6        7           8 
        {"Tgl Registrasi","Kode","Nama Perusahaan","E-mail","Username","Password","Negara","Provinsi"," Kecamatan", //8
         "Agama","No ID","Nama Studio","Detil Pot.","Credit Limit","Consigment Limit","Mata Uang"},  //15

        {"Member Type","Status","Code","Nama Perusahaan","Address","Telp No", "HP No","Place/Birthdate","Sex",
...
if(iCommand==Command.LIST||iCommand == Command.FIRST || iCommand == Command.PREV ||
  iCommand == Command.NEXT || iCommand == Command.LAST || iCommand == Command.BACK){
	try{
		srcMemberReg = (SrcMemberReg)session.getValue(SessMemberReg.SESSION_SRC_MEMBERCOMPANY);
                
	}catch(Exception e){
		srcMemberReg = new SrcMemberReg();
	}

	if(session.getValue(SessMemberReg.SESSION_SRC_MEMBERCOMPANY)==null){
		srcMemberReg = new SrcMemberReg();
	}

	session.putValue(SessMemberReg.SESSION_SRC_MEMBERCOMPANY,srcMemberReg);
}
...
int vectSize = SessMemberReg.countCompanyMember(0,recordToGet,whContacts,wheress);
...
listContact = SessMemberReg.searchCompanyMember(srcMemberReg,start,recordToGet); 
...
function cmdAdd(){
	...
	document.frmmemberreg.action="memberRegCompany_edit.jsp";
...
function cmdAsk(oidContact){
	...
	document.frmmemberreg.prev_command.value="<%=prevCommand%>";
	document.frmmemberreg.action="memberCompany_list.jsp";
	document.frmmemberreg.submit();
}
...
function cmdBack(){
	...
	document.frmmemberreg.action="srcMemberCompany.jsp";
	document.frmmemberreg.submit();
	}
