<%-- 
    Document   : pofromprmaterial_list
    Created on : Feb 20, 2014, 2:31:07 PM
    Author     : dimata005
--%>
...
{
	{"Gudang","PO dari Purchase Request","Tidak ada data order pembelian ..."},
	{"Warehouse","Purchase Request","No Data Available"}
};
...
public String drawList(int language,Vector objectClass,int start,int docType,I_DocStatus i_status)
{
	...
	{
		result = "<div class=\"msginfo\">&nbsp;&nbsp;"+textMainHeader[language][2]+"</div>";
	}
	return result;
}
%>
...
vectSize = sessPurchaseRequest.xGetCountPurchaseRequestMaterial(srcPurchaseRequest,docType,0,whereClausex);
if(iCommand==Command.FIRST || iCommand==Command.NEXT || iCommand==Command.PREV || iCommand==Command.LAST || iCommand==Command.LIST){
	start = ctrlPurchaseRequest.actionList(iCommand,start,vectSize,recordToGet);
}

Vector records = sessPurchaseRequest.searchPurchaseRequestForPOMaterial(srcPurchaseRequest,docType,start,recordToGet,0,whereClausex);
...
function cmdAdd(){
	...
	document.frm_requestmaterial.action="create_pofrompr.jsp";
...
function cmdEdit(oid){
	...
	document.frm_requestmaterial.start.value=0;
	document.frm_requestmaterial.approval_command.value="<%=Command.APPROVE%>";
	document.frm_requestmaterial.command.value="<%=Command.EDIT%>";
	document.frm_requestmaterial.action="create_pofrompr.jsp";
	document.frm_requestmaterial.submit();*/
        //window.open("prmaterial_edit.jsp?hidden_receive_id", '_blank');
        var strvalue  = "prmaterial_edit.jsp?hidden_material_request_id="+oid+"&command=3";
...
function cmdBack(){
	...
	document.frm_requestmaterial.action="srcprforpomaterial.jsp";
	document.frm_requestmaterial.submit();
}
			  ...
			  <tr align="left" valign="top">
				<td height="18" valign="top" colspan="3">
				    <table width="44%" border="0" cellspacing="0" cellpadding="0">
                                        <tr>
                                          <%if(privAdd){%>
                                            <a class="btn-primary btn-lg" href="javascript:cmdAdd()"><i class="fa fa-plus-circle"> &nbsp;<%=ctrLine.getCommand(SESS_LANGUAGE,poTitle,ctrLine.CMD_ADD,true)%></i></a>
                                          <%}%>
                                            <a class="btn-primary btn-lg" href="javascript:cmdBack()"><i class="fa fa-backward"> &nbsp;<%=ctrLine.getCommand(SESS_LANGUAGE,poTitle,ctrLine.CMD_BACK_SEARCH,true)%></i></a>
                                        </tr>
                                      </table>
				</td>
			  </tr>
