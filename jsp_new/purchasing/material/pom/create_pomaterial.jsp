...
<%!
	public static final String textListGlobal[][] = {
		{"Buat PO", "Kembali ke Minimum Stock", "Daftar PO"},
    ...
    public static final String textListMaterialHeader[][] =
    {
        ...
        {"No","Code","Product Name","Qty PO","Supplier Name","Product Order List","Order Location"}
    };
	...
	Vector vt_lokasi = PstLocation.list(0, 0, where, PstLocation.fieldNames[PstLocation.FLD_NAME]);
		...
		for (int i = 0; i < list.size(); i++){
            ...
            Vector vt_supp = PstMatVendorPrice.getContactByOidMaterial(material.getOID());
			
			/** kondisi ini digunakan untuk material yang belum memiliki supplier */
			if(vt_supp.size() == 0) {
				vt_supp = PstMatVendorPrice.getContactByOidMaterial(0);
			}
            ...
            if(vt_supp!=null && vt_supp.size()>0){
                for(int d=0; d<vt_supp.size(); d++){
                    ContactList cnt = (ContactList)vt_supp.get(d);
                        //update opie-eyek 20121226 agar supplier default menjadi yang di pilih
                        oid_supplier = Long.toString(supplier_id_select);
            ...
            rowx.add(ControlCombo.draw("supplier_order_name",null,oid_supplier,val_supplier,key_supplier,"","formElemen"));
...
<%
    ...
    Vector list = SessMaterial.getListItemForPO(vectQty);
    if(finish==1){
        vectQty = new Vector();
        if(txtqtypo.length > 0){
            for(int k=0;k < txtqtypo.length;k++){
            ...
            if(vectQty.size() > 0){
                PstPurchaseOrder.autoInsertPo(vectQty);
            }else{
                if(txtqtypo.length > 0){
                    for(int k=0;k < txtqtypo.length;k++){
                        long oidMaterial = Long.parseLong(txtqtypo[k]);
...
<script language="JavaScript">
	function cmdSave()
	{
		...
		document.frm_newpo.action="create_pomaterial.jsp";
