...
public static final String textListMaterialHeader[][] = {
    {"NO","SKU","NAMA BARANG","UNIT","HARGA BELI","QTY BELI TERAKHIR","DOKUMENT PENERIMAAN TERAKHIR"},
    {"NO","CODE","NAME PRODUCT","UNIT","COST","LAST ORDER QTY","LAST RECEIVE"}
};

public String drawListMaterial(int currency,int language,Vector objectClass,int start, boolean sts, double curr){
    String result = "";
        ...
        for(int i=0; i<objectClass.size(); i++){
            ...
            double qtyLast = 0;
             for(int r=0; r<listLastReceive.size(); r++){
                  ...
                  dokumentLast="<a href=\"javascript:viewDocumentReceive('"+matreceiveitem.getReceiveMaterialId()+"',0)\">"+matreceiveitem.getRecCode()+"</a>";
                  qtyLast = matreceiveitem.getQty();
            }
            ...
            try{
                if(hpp_calculate==1){
                    defaultCost = material.getAveragePrice();
                }else{
                    if(sts){
                        defaultCost = matVendorPrice.getOrgBuyingPrice()*exchangeRateLastReceive;
                    }else{
                        defaultCost = material.getDefaultCost()*exchangeRateLastReceive;
                    }
                }
            }   
            catch(Exception exc){
            ...
            rowx.add(""+FRMHandler.userFormatStringDecimal(defaultCost)+"");
            rowx.add(""+qtyLast);
            rowx.add(""+dokumentLast);
            lstData.add(rowx);
...
*/
int vectSize = 0;//PstMaterial.getCountMaterialItem(oidVendor,objMaterial);
//if(vectSize==0)
if(showAllGoods>0){
    vectSize = PstMaterial.getCountMaterialItem(0,objMaterial);
}else{
    vectSize = PstMaterial.getCountMaterialItem(oidVendor,objMaterial);
}
...
int hpp_calculate =Integer.parseInt(PstSystemProperty.getValueByName("HPP_CALCULATION"));
boolean sts = true;
Vector vect = new Vector();//PstMaterial.getListMaterialItem(oidVendor,objMaterial,start,recordToGet, orderBy);
if(showAllGoods>0){
    vect = PstMaterial.getListMaterialItem(0,objMaterial,start,recordToGet, orderBy);
    sts = false;
}else{
    vect = PstMaterial.getListMaterialItem(oidVendor,objMaterial,start,recordToGet, orderBy);
}
...
if(vectSize == 1 && automaticSelected==0) {
    ...
    MatVendorPrice matVendorPrice = (MatVendorPrice)vData.get(4);
    try {
            material = PstMaterial.fetchExc(material.getOID());     
    }
    catch(Exception e) {
...
%>
    ...
    self.opener.document.forms.frm_purchaseorder.<%=FrmPurchaseOrderItem.fieldNames[FrmPurchaseOrderItem.FRM_FIELD_PRICE_KONVERSI]%>.value=matPrice;
...
function viewDocumentReceive(idRcv, idPo){
	...
	if(parseInt(idPo) != 0) {
                document.frmvendorsearch.action="../../../warehouse/material/receive/receive_wh_supp_material_edit.jsp?hidden_receive_id="+idRcv+"&start=0&approval_command=44&command=3";
	}
	else {
                document.frmvendorsearch.action="../../../warehouse/material/receive/receive_wh_supp_material_edit.jsp?hidden_receive_id="+idRcv+"&start=0&approval_command=44&command=3";
	}
    <%}else{%>
        if(parseInt(idPo) != 0) {
                document.frmvendorsearch.action="../../../warehouse/material/receive/receive_wh_supp_po_material_edit.jsp?hidden_receive_id="+idRcv+"&start=0&approval_command=44&command=3";
	}
	else {
                document.frmvendorsearch.action="../../../warehouse/material/receive/receive_wh_supp_material_edit_old.jsp?hidden_receive_id="+idRcv+"&start=0&approval_command=44&command=3";
	} 
    <%}%>     
    document.frmvendorsearch.submit();    
}
        ...
        <tr> 
          <td> 
            <form name="frmvendorsearch" method="post" action="">
              ...
              <input type="hidden" name="source_link2" value="materialdosearch.jsp">
              <table width="100%" border="0" cellspacing="1" cellpadding="1">
                ...
                <tr> 
                  <td colspan="2"><%=drawListMaterial(currType,SESS_LANGUAGE,vect,start,sts,curr)%></td>
                </tr>
                <tr> 
