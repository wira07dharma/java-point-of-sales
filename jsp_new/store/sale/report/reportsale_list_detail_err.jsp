<%-- 
    Document   : reportsale_list_detail_err
    Created on : Aug 8, 2015, 9:30:34 AM
    Author     : dimata005
--%>
...
public Vector drawList(int language, Vector objectClass, SrcSaleReport srcSaleReport, int viewImage,String approot) {
        ...
        ctrlist.addHeader("Date", "13%");
        ctrlist.addHeader("Bill", "10%");
        ctrlist.addHeader("Menu", "20%");
        ctrlist.addHeader("Qty", "5%");
        ctrlist.addHeader("Price", "10%");
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            rowx.add(""+billlDetail.getItemPrice());
            rowx.add(""+billlDetail.getTotalPrice());
...
Vector records = SaleReportDocument.getListReportError(srcSaleReport,0,0,SaleReportDocument.LOG_MODE_CONSOLE);
...
function cmdSort(method,desc){
	...
	document.frm_reportsale.action="reportsale_list_detail_err.jsp";
...
function cmdBack(){
	...
	document.frm_reportsale.global_group.value=<%=global_group%>;
	document.frm_reportsale.action="src_report_err.jsp";
	document.frm_reportsale.submit();
}
