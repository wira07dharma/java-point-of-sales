...
{
	{"NO","TGL","NOMOR","KONSUMEN","RETUR BRUTO","DISC","TAX","SERVICE","RETUR NETTO","DP DEDUCTION"},
	{"NO","DATE","NUMBER","CUSTOMER","RETURN BRUTO","DISC","TAX","SERVICE","RETURN NETTO","DP DEDUCTION"}
};
...
public static final String textListTitleHeader[][] = 
{
	...
	{"Daily Sales Recapitulation Report","SALES RECAPITULATION REPORT PER SHIFT","No transaction data available ..","LOCATION","SHIFT","Report","Print Daily Transaction ","TYPE","Currency Type"}
};

public static final String textGlobal[][] =
{
           {"Semua Location","Tipe","Return Penjualan Cash","Return Penjualan Credit","Mata Uang","Tanggal"},
           {"All Location","Type","Return Sales Cash","Return Credit Sales","Currency","Date"}
};

public String drawList(int language,Vector objectClass,SrcSaleReport srcSaleReport, int start, long currencyId) {
	...
	if(objectClass!=null && objectClass.size()>0){   
		...
		System.out.println("Contact not found ...");
			...
			//double totDp = PstPendingOrder.getDpValue(billMain.getOID());												
            //rowx.add("<div align=\"right\">"+FRMHandler.userFormatStringDecimal(totDp)+"</div>");															
...
public String drawListReturnPayment(Vector vListPayment) 
{
	String sResult = "";
		...
		for(int i=0; i<iListPaymentCount; i++)
		{
			...
			if(vObj!=null && vObj.size()>0)
			{
				...
				for(int j=0; j<vObj.size(); j++)
				{
					...
					if(objCashPayment.getRate() == 1) {
						defaultCurrency = strCurrency;
					}
				}
...
public String drawListReturnPaymentDinamis(Vector vListPayment)
{
	String sResult = "";
	if(vListPayment!=null && vListPayment.size()>0)
	{
...
try {
	...
	System.out.println("Exc when fetch srcSaleReport : " + e.toString());
}
...
try {
	...
	location.setName(textGlobal[SESS_LANGUAGE][0]);
}
...
function cmdEdit(oid)
{
	...
	document.frm_reportsale.action="return_invoice_edit.jsp";
	document.frm_reportsale.submit();
}

function cmdListFirst(){
	document.frm_reportsale.command.value="<%=Command.FIRST%>";
	document.frm_reportsale.action="reportlistreturn.jsp";
	document.frm_reportsale.submit();
}
...
function cmdBack(){
	...
	document.frm_reportsale.action="src_reportreturn.jsp";
	document.frm_reportsale.submit();
	//history.back();
}
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">  
        <tr> 
          <td height="20" class="mainheader"><!-- #BeginEditable "contenttitle" --> 
            &nbsp;Laporan Penjualan > Retur Per Invoice<!-- #EndEditable --></td>
        </tr>
        <tr> 
          <td><!-- #BeginEditable "content" --> 
            <form name="frm_reportsale" method="post" action="">
              ...
              <table width="100%" cellspacing="0" cellpadding="3">
                ...
                <tr align="left" valign="top"> 
                  <td height="14" valign="middle" colspan="3" class="command"> 
                    <b><%=textGlobal[SESS_LANGUAGE][5]%> : <%=Formater.formatDate(srcSaleReport.getDateFrom(), "dd-MM-yyyy")%> s/d <%=Formater.formatDate(srcSaleReport.getDateTo(), "dd-MM-yyyy")%></b> </td>
                </tr>
                <tr align="left" valign="top"> 
                  <td height="22" valign="middle" colspan="3"><b><%=textGlobal[SESS_LANGUAGE][1]%>: <%=(iSaleReportType==PstBillMain.TRANS_TYPE_CASH) ? textGlobal[SESS_LANGUAGE][2] : textGlobal[SESS_LANGUAGE][3]%></b></td>
                </tr>
                <tr align="left" valign="top"> 
                  <td height="22" valign="middle" colspan="3"><b><%=textListTitleHeader[SESS_LANGUAGE][8]%> : <%=strCurrencyType%></b></td>
                </tr>
                <tr align="left" valign="top"> 
                  <td height="22" valign="middle" colspan="3"><%=drawList(SESS_LANGUAGE, records, srcSaleReport, start, srcSaleReport.getCurrencyOid())%></td>
                </tr>
				...
				if(records!=null && records.size()>0 && iSaleReportType == PstCashPayment.CASH) {
                                        ...
                                        if(paymentDinamis == PstPaymentSystem.USING_PAYMENT_DINAMIS) {
                                            ...
                                            //vResult = objPstCashPaymentDinamis.getListPaymentDinamis(srcSaleReport, srcSaleReport.getLocationId(), srcSaleReport.getDateFrom(), srcSaleReport.getDateTo(), PstBillMain.TYPE_RETUR, iSaleReportType);
                                            vResult = objPstCashPaymentDinamis.getListPaymentDinamis(srcSaleReport,PstBillMain.TYPE_RETUR, iSaleReportType);
                                            strTemp = drawListReturnPaymentDinamis(vResult);
                                        }else{
                                            PstCashPayment objPstCashPayment = new PstCashPayment();
                                            vResult = objPstCashPayment.getListPayment(srcSaleReport, srcSaleReport.getLocationId(), srcSaleReport.getDateFrom(), srcSaleReport.getDateTo(), PstBillMain.TYPE_RETUR, iSaleReportType);
                                            strTemp = drawListReturnPayment(vResult);
                                        }
					/*PstCashPayment objPstCashPayment = new PstCashPayment();
					Vector vResult = objPstCashPayment.getListPayment(srcSaleReport, srcSaleReport.getLocationId(), srcSaleReport.getDateFrom(), srcSaleReport.getDateTo(), PstBillMain.TYPE_RETUR, iSaleReportType);
					String strTemp = drawListReturnPayment(vResult);*/
				%>
				<tr align="left" valign="top"> 
