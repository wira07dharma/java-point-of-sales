<%-- 
    Document   : reportsale_list_detail_global_100_100
    Created on : Sep 27, 2016, 12:10:42 PM
    Author     : dimata005
--%>
...
public static final String textListTitleHeader[][] = {
    ...
    {"REPORT TOP 100 TRANSACTION ", "DETAIL TRANSACTION REPORT OF ", "No transaction data available", "LOCATION", "SHIFT", 
...
if(iCommand==Command.BACK || iCommand==Command.FIRST || iCommand==Command.PREV || iCommand==Command.NEXT || iCommand==Command.LAST || iCommand==sortCommand) {
    ...
    if(srcSaleReport.getBillDateStatus()==1){
        srcSaleReport.setDateFrom(null);
    }
    srcSaleReport.setSortBy(SrcSaleReport.SORT_BY_TOTAL_QTY);
    srcSaleReport.setDescSort(SrcSaleReport.SORT_DESC);
...
function cmdSort(method,desc){
	...
	document.frm_reportsale.action="reportsale_list_detail_global_100.jsp";
...
function cmdBack(){
	...
	document.frm_reportsale.global_group.value=<%=global_group%>;
        if(<%=fromglobal%>==1){
            document.frm_reportsale.action="src_reportsale_detail_global_100.jsp?urutan=1";
        }else{
            document.frm_reportsale.action="src_reportsale_detail_global_100.jsp?urutan=1";
        }
...
function printFormExcel(){
     window.open("reportsale_list_detail_global_100_excel.jsp?view_photo=<%=srcSaleReport.getImageView()%>&approot=<%=approot%>&hidden_type_sort=<%=hidden_type_sort%>","salereport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
	
}


function printFormHtml(){
     window.open("reportsale_list_detail_global_100_html.jsp?view_photo=<%=srcSaleReport.getImageView()%>&approot=<%=approot%>&global_group=<%=global_group%>","salereport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
