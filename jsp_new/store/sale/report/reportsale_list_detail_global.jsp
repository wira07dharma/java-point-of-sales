...
//if(iCommand==Command.BACK || iCommand==Command.FIRST || iCommand==Command.PREV || iCommand==Command.NEXT || iCommand==Command.LAST ){
    ...
    int tempGB = FRMQueryString.requestInt(request, "FRM_FLD_GROUP_BY");
//if (tempGB==SrcSaleReport.GROUP_BY_SALES && salesId==0 && fromglobal==0){
    //srcSaleReport.setGroupBy(SrcSaleReport.GROUP_BY_SALES);
    //}else{
    //srcSaleReport.setGroupBy(SrcSaleReport.GROUP_BY_ITEM);
//}
    srcSaleReport.setGroupBy(SrcSaleReport.GROUP_BY_ITEM);

    if (hidden_type_sort == 1) {
...
//srcSaleReport.setSortBy(SrcSaleReport.SORT_BY_ITEM);
    ...
    if (useForGreenbowl.equals("1")) {
        ...
        records = SaleReportDocument.getList(srcSaleReport, 0, 0, SaleReportDocument.LOG_MODE_CONSOLE);
    }
...
// pembuatan invoice internal
    ...
    if (useForGreenbowl.equals("1")) {
        ...
        list = drawList(SESS_LANGUAGE, records, srcSaleReport, srcSaleReport.getImageView(), approot, salesId);
    }
    String str = "";
    ...
    <head>
        ...
        <script language="JavaScript">
            ...
            function cmdSort(method, desc) {
                // alert("aa");
                document.frm_reportsale.hidden_type_sort.value = "1";
            ...
            function printForm() {
                window.open("<%=approot%>/servlet/com.dimata.posbo.report.sale.SaleReportPdfDetail?view_photo=<%=srcSaleReport.getImageView()%>&approot=<%=approot%>&hidden_type_sort=<%=hidden_type_sort%>", "salereport", "scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
                    ...
                    function printFormExcel() {
                        window.open("reportsale_list_detail_global_excel.jsp?view_photo=<%=srcSaleReport.getImageView()%>&approot=<%=approot%>&hidden_type_sort=<%=hidden_type_sort%>", "salereport", "scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");

                    }


                    function printFormHtml() {
                        window.open("reportsale_list_detail_global_html.jsp?view_photo=<%=srcSaleReport.getImageView()%>&approot=<%=approot%>&global_group=<%=global_group%>", "salereport", "scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
