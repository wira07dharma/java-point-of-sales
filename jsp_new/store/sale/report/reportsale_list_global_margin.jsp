		...
		for(int i=0; i<objectClass.size(); i++) {
			...
			//textListMaterialHeader
                        ...
                        if(materialId!=0){                        
                            ...
                            rowx.add("<div align=\"left\"><a href=\"javascript:cmdEditMat('"+materialId+"')\" >"+nameValue+"</a></div>");
                        }else{
                            rowx.add("<div align=\"left\">"+nameValue+"</div>");
                        }
                        ...
                        if(materialId!=0){
                            rowx.add("<div align=\"right\"><a href=\"javascript:cmdCost('"+Formater.formatDate(srcSaleReport.getDateTo(), "dd-MM-yyyy")+"','"+Formater.formatDate(srcSaleReport.getDateFrom(), "dd-MM-yyyy")+"','"+srcSaleReport.getLocationId()+"','"+materialId+"')\" >"+FRMHandler.userFormatStringDecimal(costValue)+"</a></div>");
                        }else{
...
function cmdSort(method,desc)
{
	...
	document.frm_reportsale.action="reportsale_list_global_margin.jsp";
...
function cmdEditMat(oid)
{
    //document.frm_reportsale.hidden_material_id.value=oid;
    //document.frm_reportsale.start.value=0;
    //document.frm_reportsale.approval_command.value="<//%=Command.APPROVE%>";					
    //document.frm_reportsale.command.value="<//%=Command.EDIT%>";
    var win = window.open("../../../master/material/material_main.jsp?hidden_material_id="+oid+"&start=0&approval_command=44&command=3", '_blank');
    win.focus();
    //document.frm_reportsale.action="../../../master/material/material_main.jsp";
    //document.frm_reportsale.target="_blank";
    //document.frm_reportsale.submit();
}
...
function cmdBack()
{
	...
	document.frm_reportsale.global_group.value=<%=global_group%>;
	document.frm_reportsale.action="src_reportsale_global_margin.jsp";
	document.frm_reportsale.submit();
}
function cmdCost(to,from,location,materialid){
    window.open("form_cost.jsp?datefrom='"+from+"'&dateto='"+to+"'&location="+location+"&materialIdxx="+materialid,"salereport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
}

function printForm(){
    //window.open("<%=approot%>/servlet/com.dimata.posbo.report.sale.SaleReportPdfMargin","salereport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
    window.open("reportsale_list_global_margin_html.jsp","salereport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
}

function printFormExcel(){
    window.open("reportsale_list_global_margin_excel.jsp","salereport","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
}
                      ...
                      <tr> 
                        <td width="61%"> 
                          <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr> 
                              ...
                              <td class="command" nowrap width="95%"><a class="btn btn-primary" href="javascript:cmdBack()"><i class="fa fa-arrow-left"></i> <%=ctrLine.getCommand(SESS_LANGUAGE,textListTitleHeader[SESS_LANGUAGE][5],ctrLine.CMD_BACK_SEARCH,true)%></a></td>
                            </tr>
                          </table>
                        </td>
                        <td width="39%"> <table width="100%" border="0" cellpadding="0" cellspacing="0">
                            ...
                            <%if(vectSize>0){%>
                              ...
                              <td width="45%" nowrap>&nbsp; <a href="javascript:printForm()" class="command" >
                                <%=textListTitleHeader[SESS_LANGUAGE][6]+""+SrcSaleReport.reportType[SESS_LANGUAGE][srcSaleReport.getTransType()]%>
                            </a></td>
                            ...
                            session.putValue(SaleReportDocument.SALE_REPORT_DOC_MARGIN_PDF,listPdf);
                            }catch(Exception e){
                                e.printStackTrace(); 
                                }
                            %>
