...
public String drawList(JspWriter outObj, int language,Vector objectClass,SrcSaleReport srcSaleReport, int start, Vector vectPay, long currencyId, int typePayment,int integrasi,String dataAddress) {
	...
	if(objectClass!=null && objectClass.size()>0){   
		...
		ctrlist.dataFormat(textListMaterialHeader[language][0],"3%","0","0","center","bottom");//no
                ctrlist.dataFormat(textListMaterialHeader[language][2],"7%","0","0","center","bottom");//nomor
                ctrlist.dataFormat(textListMaterialHeader[language][1],"7%","0","0","center","bottom");//tanggal
                ...
                if (integrasi==1){
                    ctrlist.dataFormat("# <input type='checkbox' id='checkAll'>","10%","0","0","center","bottom");
                }
		...
		for(int i=0; i<objectClass.size(); i++)	{
            ...
            if (integrasi==1){
                ...
                //try {
                    ...
                    //if (!result.next()){
                        rowx.add("<div align=\"right\"><input type='checkbox' value='"+billMain.getOID()+"' class='check' data-oid='"+billMain.getOID()+"'></div>");
                    //}
                    //conn.close();
...
<%
//GET URL API SAVE AR
String saveArApi = PstSystemProperty.getValueByName("INTEGRASI_URL_SAVE_AR_API");
String approotARApi = "";
String proccessPage = "";
if(saveArApi.length() > 0){
    String[] apiComponent = saveArApi.split(";");
    if(apiComponent.length >= 2){
        approotARApi = apiComponent[0];
        proccessPage = apiComponent[1];
    }
}
...
int integration = 0;
integration = Integer.parseInt(PstSystemProperty.getValueByName("INTEGRASI_DATABASE"));
...
function cmdListFirst(){
	...
	document.frm_reportsale.action="reportlistsale_ar_integ.jsp";
	document.frm_reportsale.submit();
}
...
function printExcel2(){
    window.open("<%=approot%>/servlet/com.dimata.printman.SalePerInvoiceExcel2?sale_type=<%=iSaleReportType%>&command=<%=Command.LIST%>","corectionwh","scrollbars=yes,height=600,width=800,status=no,toolbar=no,menubar=yes,location=no");
...
<%if(menuUsed == MENU_PER_TRANS){%>
              ...
              <table width="100%" cellspacing="0" cellpadding="3">
                ...
                <tr align="left" valign="top">
                  <td height="22" valign="middle" colspan="3">
                      <a href="javascript:printExcel()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image10','','<%=approot%>/images/BtnSearchOn.jpg',1)" id="aSearch"><img name="Image10" border="0" src="<%=approot%>/images/BtnSearch.jpg" width="24" height="24" alt="Cari Laporan"></a>
                      <a href="javascript:printExcel()">download via excel</a>|
                      <a href="javascript:printExcel2()" onMouseOut="MM_swapImgRestore()" onMouseOver="MM_swapImage('Image10','','<%=approot%>/images/BtnSearchOn.jpg',1)" id="aSearch"><img name="Image10" border="0" src="<%=approot%>/images/BtnSearch.jpg" width="24" height="24" alt="Cari Laporan"></a>
                      <a href="javascript:printExcel2()">download via excel 2</a>
                  </td>
                        ...
                        <td width="61%"> 
                          <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr> 
                              ...
                              <td class="command" nowrap width="95%">
                                  <a class="btn btn-primary" href="javascript:cmdBack()"><i class="fa fa-arrow-left"></i> <%=ctrLine.getCommand(SESS_LANGUAGE,textListTitleHeader[SESS_LANGUAGE][5],ctrLine.CMD_BACK_SEARCH,true)%></a> 
                                  <% if (integration==1){ %>
                                  &nbsp;
                                    &nbsp;<button class="btn btn-primary" type="button" id="sendData" ><i class="fa fa-check"></i> Send Data</button>
                                  <% } %>
                                  &nbsp;
                                  &nbsp;<button class="btn btn-primary" type="button" id="printReport" ><i class="fa fa-print"></i> Print Report</button>
                                  &nbsp;<button class="btn btn-primary" type="button" id="printExcelReport" ><i class="fa fa-print"></i> Print Excel Report</button>
                              </td>
  ...
  <tr> 
    <td colspan="2" height="20"> <!-- #BeginEditable "footer" --> 
      ...
      <script type="text/javascript" src="../../../styles/bootstrap-3.3.7/js/jquery-3.1.0.min.js"></script>
    ...
    <script type="text/javascript">
		...
		$(this).getData({
		   ...
		   ajaxDataType	: dataType
            ...
            var sendDataAPI = function(data){
                var approotAPI = "<%= approotARApi %>";
                var dataSendApi = {
                    "command" : "save",
                    "FRM_FIELD_DATA_FOR" : "saveAR",
                    "dataObject" : data.dataObject,
                    "username" : "admin",
                    "password" : "dsj4qu33nt4nd0r123"
                }
                var onSuccessApi = function(data){
                    
                };
                var onDoneApi = function(data){
                    alert(data.message);
                    alert(data.success);
                    if(data.success){
                        alert("ok");
                ...
                getDataFunction(onDoneApi, onSuccessApi, approotAPI, command, dataSendApi, "<%= proccessPage %>", null, false, "json");
            };
            
            var checkSubmit= function(elementId){
                $(elementId).click(function(){
                    valueCheck = "";
                    command = <%= Command.SAVE%>;
                    dataFor = "inserttoother";
			...
			if($(this).is(":checked")){
				...
				valueCheck += ","+$(this).val();
                    ...
                    var confirmText = "Are you sure want to send these data?";
                    if(valueCheck.length == 0){
			alert("Please select the data");
				...
				"FRM_FIELD_OID_SEND"	    : valueCheck,
                                "FRM_FIELD_ADDRESS"	    : address,
			    ...
			    var onSuccess = function(data){
                                ...
                                //alert("Success Transfer Data..")
                                //location.reload();
			    };			   
                            var onDone = function(data){
                                sendDataAPI(data);
                            };			    
			    getDataFunction(onDone, onSuccess, approot, command, dataSend, "AjaxReportListSale", null, false, "json");
            ...
            var checkAll = function (elementId){
            ...
            var printHtml = function(elementId){
                $(elementId).click(function(){
                    var command = "<%= iCommand%>";
                    var sale_type= "<%= iSaleReportType%>";
                    var start = "<%= start%>";
                    var hidden_billmain_id = "<%= billMainID%>";
                     window.open("reportlistsalse_html.jsp?command="+command+"&sale_type="+sale_type+"&start="+start+"&hidden_billmain_id="+hidden_billmain_id+"");
                    //alert("command="+command+"&sale_type="+sale_type+"&start="+start+"&hidden_billmain_id="+hidden_billmain_id+"");
                });
            };
            
             var printExcelReport = function(elementId){
                $(elementId).click(function(){
                    var command = "<%= iCommand%>";
                    var sale_type= "<%= iSaleReportType%>";
                    var start = "<%= start%>";
                    var hidden_billmain_id = "<%= billMainID%>";
                     window.open("reportlistsalse_excel.jsp?command="+command+"&sale_type="+sale_type+"&start="+start+"&hidden_billmain_id="+hidden_billmain_id+"");
                });
            };
            
            checkSubmit("#sendData");
            checkAll("#checkAll");
            printHtml("#printReport");
            printExcelReport("#printExcelReport");
        });
    </script>
