...
{
	{"Laporan ","NOMOR INVOICE","TANGAAL INVOICE","NAMA CUSTOMER","(tidak tercatat)"},
	{"Report ","INVOICE NUMBER","INVOICE DATE","CUSTOMER NAME","(unspecified)"}
};
	...
	if(vListPayment!=null && vListPayment.size()>0)
	{
		...
		for(int i=0; i<iListPaymentCount; i++)
		{
			...
			if(vObj!=null && vObj.size()>0)
			{
				...
				for(int j=0; j<vObj.size(); j++)
				{
					CashCreditPaymentsDinamis objCashCreditPayments = (CashCreditPaymentsDinamis) vObj.get(j);

					strPaymentType = PstPaymentSystem.getTypePayment(objCashCreditPayments.getPaymentType());
					String strNumber = String.valueOf(j+1);
					String strCurrency = String.valueOf(hashCurrType.get(""+objCashCreditPayments.getCurrencyId()));
					String strAmount = FRMHandler.userFormatStringDecimal( (objCashCreditPayments.getAmount()/objCashCreditPayments.getRate() ));
					String strRate = FRMHandler.userFormatStringDecimal(objCashCreditPayments.getRate());
				...
				strContent = strContent + generateItemFooter("TOTAL "+strPaymentType, FRMHandler.userFormatStringDecimal(dPaymentPerType));
				strContent = strContent + generateBlankSpace();
				dTotalPayment = dTotalPayment + dPaymentPerType;
			}
		}
...
if(iCommand==Command.EDIT || iCommand==Command.ASK || iCommand==Command.DELETE){
   try
   {
		...
		if(creditPaymentMain.getBillMainId()>0)
            ...
            totTrans = totTrans - billMain.getDiscount();
            totTrans = totTrans + billMain.getServiceValue();
            totTrans = totTrans + billMain.getTaxValue();
            ...
            //buat return transaksi
            //cek apakah ada transaksi dengan parent id
            String whereClause="";
            whereClause=PstBillMain.fieldNames[PstBillMain.FLD_PARENT_ID]+"="+billMain.getOID();
            Vector vResultReturn = PstBillMain.getTransactionReturnList(whereClause, "");//PstCashCreditPayment.getTotalCreditPaymentNotInPayNow(creditPaymentMain.getBillMainId(),oidGroup);//objPstCreditPaymentMain.getListCreditPayment(oidGroup);
            int countReturn = vResultReturn.size();
            if(vResultReturn!=null && vResultReturn.size()>0)
                for(int i=0; i<countReturn; i++)
		{
                    BillMain billMainReturn = (BillMain) vResultReturn.get(i);
                    //BillMain billMainReturn = new BillMain();
                    //billMainReturn = PstBillMain.fetchExc(creditPaymentMain.getBillMainId());
                    amountReturn = PstBillDetail.getSumTotalItem(PstBillDetail.fieldNames[PstBillDetail.FLD_BILL_MAIN_ID]+"="+billMainReturn.getOID());
                    amountReturn = amountReturn - billMainReturn.getDiscount();
                    amountReturn = amountReturn + billMainReturn.getServiceValue();
                    amountReturn = amountReturn + billMainReturn.getTaxValue();

                    totReturn=totReturn+amountReturn;
                }
          }
    }
	catch(Exception e)
...
function cmdBack()
{
	...
	document.frminvoice.action="reportlistsalepaymentcredit.jsp";
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">  
        <tr> 
          <td height="20" class="mainheader"><!-- #BeginEditable "contenttitle" --> 
            &nbsp;Laporan Penjualan > Pelunasan Piutang<!-- #EndEditable --></td>
        </tr>
        <tr> 
              ...
              <table width="100%" cellspacing="0" cellpadding="3">
                      ...
                      <tr> 
                        <td width="53%"> 
                          <table width="60%" border="0" cellspacing="1" class="listgen">
                            ...
                            <tr> 
                              <td align="left" valign="top" width="24%" class="listgensell"><b><font color="#000000">Tanggal 
                                Transaksi</font></b></td>
                              <td align="center" valign="top" width="3%" class="listgensell"><b><font color="#000000">:</font></b></td>
                             ...
                             <tr>
                              ...
                              <td align="left" valign="top" width="20%" class="listgensell"><b><font color="#000000">Return</font></b></td>
                              <td align="center" valign="top" width="3%" class="listgensell"><b><font color="#000000">:</font></b></td>
                              <td align="right" valign="top" width="17%" class="listgensell"><font color="#000000"><%=FRMHandler.userFormatStringDecimal(totReturn)%></font></td>
                            </tr>
                            <tr> 
                              <td align="left" valign="top" width="24%" class="listgensell"><b><font color="#000000">Nama 
                                Member</font></b></td>
                              <td align="center" valign="top" width="3%" class="listgensell"><b><font color="#000000">:</font></b></td>
                            ...
                            <tr> 
                              <td align="left" valign="top" width="24%" class="listgensell"><b><font color="#000000">Invoice 
                                Referensi</font></b></td>
                              <td align="center" valign="top" width="3%" class="listgensell"><b><font color="#000000">:</font></b></td>
                              ...
                              <td align="left" valign="top" width="20%" class="listgensell"><b><font color="#000000">Saldo 
                                Akhir</font></b></td>
                              <td align="center" valign="top" width="3%" class="listgensell"><b><font color="#000000">:</font></b></td>
                              <td align="right" valign="top" width="17%" class="listgensell"><font color="#000000"><%=FRMHandler.userFormatStringDecimal(totTrans - (totTerbayar + totBayar+totReturn))%></font></td>
                            </tr>
                          </table>
                        </td>
                      </tr>
				...
				if(creditPaymentMain.getOID()!=0)
				{
                                        ...
                                        if(paymentDinamis == PstPaymentSystem.USING_PAYMENT_DINAMIS) {
                                            vResult = objPstCreditPaymentMain.getListCreditPaymentDinamis(oidGroup);
                                            strTemp = drawListCreditPaymentDinamis(vResult);
                                        }else{
                                            vResult = objPstCreditPaymentMain.getListCreditPayment(oidGroup);
                                            strTemp = drawListCreditPayment(vResult);
                                        }
                ...
                <tr align="left" valign="top"> 
                  <td height="18" valign="top"> 
					...
					%>
                    <%= ctrLine.drawImage(iCommand, iErrCode, "")%> </td>
                </tr>
              </table>
            </form>
            <!-- #EndEditable --></td> 
