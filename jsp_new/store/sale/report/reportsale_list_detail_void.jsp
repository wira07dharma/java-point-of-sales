<%-- 
    Document   : reportsale_list_detail_void
    Created on : Aug 7, 2015, 10:54:25 PM
    Author     : dimata005
--%>
...
public Vector drawList(int language, Vector objectClass, SrcSaleReport srcSaleReport, int viewImage,String approot) {
        ...
        for (int i = 0; i < objectClass.size(); i++) {
            ...
            no=no+1;
            rowx.add(""+no);
            //String newTime = new SimpleDateFormat("dd/M/yyyy HH:mm").format(billlDetail.getLengthOrder());
...
Vector records = SaleReportDocument.getListReportVoid(srcSaleReport,0,0,SaleReportDocument.LOG_MODE_CONSOLE);
...
function cmdSort(method,desc){
	...
	document.frm_reportsale.action="reportsale_list_detail_void.jsp";
...
function cmdBack(){
	...
	document.frm_reportsale.global_group.value=<%=global_group%>;
	document.frm_reportsale.action="src_report_void.jsp";
	document.frm_reportsale.submit();
}
