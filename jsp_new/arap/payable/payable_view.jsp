...
<%  int appObjCodex = AppObjInfo.composeObjCode(AppObjInfo.G1_STOCK, AppObjInfo.G2_STOCK, AppObjInfo.OBJ_STOCK_POSTING);
  boolean privUnlock = userSession.checkPrivilege(AppObjInfo.composeCode(appObjCode, AppObjInfo.COMMAND_START));
%>
<%!  public static final String textGlobalTitle[][] = {
  ...
  public static final String textListHeaderDetail[][] = {
    {"Tanggal", "Lokasi", "Sistem Pembayaran", "Mata Uang", "Rate", "Jumlah dalam Mata Uang", "Jumlah", "Payment Number"},
    {"Date", "Location", "Payment System", "Currency", "Rate", "Amount in Currency", "Amount", "Payment Number"}
  };
...
//currency
  ...
  if (iCommand != Command.LIST) {
    try {
      ...
      if ((srcAccPayable.getInvoicePaymentDateStatus() != 1 && srcAccPayable.getInvoiceDateStatus() != 1)) {
        srcAccPayable.setStartDate(null);
      }
...
//records = SessAccPayable.getListAP(srcAccPayable, start, recordToGet);
  ...
  /**
   * Untuk mendapatkan besarnya daily rate per satuan default (:currency rate =
   * 1)
   */
  /**
   * Yang dibagi dengan variabel dailyRate yaitu: dr pada function
   * javascript:calculate(), totalPayment, apPayment dan amount Ini berfungsi
   * untuk mengasilkan amount yang sesuai dengan search key yang diminta (Rp
   * atau USD)
   */
...
//String whereClause2 = PstDailyRate.fieldNames[PstDailyRate.FLD_CURRENCY_TYPE_ID]+" = "+srcAccPayable.getCurrencyId();
  ...
  Vector <DailyRate> listDailyRate2 = PstDailyRate.list(0, 0, whereClause2, orderClause2);
  double dailyRate = 0;
  for(DailyRate dr2 : listDailyRate2){
  dailyRate = dr2.getSellingRate();
  }
  ...
  if (iCommand == Command.UNLOCK) {
    SessAccPayable.updatePurchaseRequest(oidPaymentSelected, 0);
  }
...
<html><!-- #BeginTemplate "/Templates/main.dwt" --><!-- DW6 -->
  <head>
    ...
    <script language=JavaScript>
      ...
      <%
          ...
          try {
            dr = (DailyRate) listDailyRate.get(0);
          } catch (Exception e) {
            //srcAccPayable = new SrcAccPayable();
            dr = new DailyRate();
          }
      ...
      function cmdOpenPayment(oidPayment, oidPaymentDetail){
      document.frap.command.value = "<%=Command.UNLOCK%>";
              document.frap.oid_payment_selected.value = oidPayment;
              document.frap.oid_payment_detail_selected.value = oidPaymentDetail;
              document.frap.action = "payable_view.jsp";
              document.frap.submit();
      ...
      var amount = parseFloat(cleanNumberFloat(document.frap.amount.value, guiDigitGroup, guiDecimalSymbol));
              var ap_saldo = Math.round(parseFloat(cleanNumberFloat(document.frap.ap_saldo.value, guiDigitGroup, guiDecimalSymbol)));
      ...
      document.frap.action = "../../warehouse/material/receive/receive_wh_supp_material_edit_old.jsp";
      <%} else {%>
  ...
  <body class="bg">    
    <table width="100%" border="0" cellspacing="0" cellpadding="0" style="background-color:#FCFDEC; height: 100%;" >
            ...
            <tr> 
              <td><!-- #BeginEditable "content" --> 
                <form name="frap" method="post" action="">
                  ...
                  <input type="hidden" name="hidden_oid_pp" value="<%=paymentInfo.getStCardNumber()%>">
                              ...
                              <table width="100%"  border="0" class="listgen" cellspacing="1" cellpadding="1">
                                ...
                                //totalRetur = SessMatReturn.getTotalReturnByReceive(srcAccPayable, 0L);
                                  ...
                                  for (int i = 0; i < records.size(); i++) {
                                    ...
                                    long oidContact = Long.parseLong((String) temp.get(7));
                                  ...
                                  <td colspan="8"> 
                                    <table width="100%" border="0"  class="listgen" cellspacing="1" cellpadding="1">
                                      <tr class="listgentitle" align="center"> 
                                        <td width="15%"><strong><%=textListHeaderDetail[SESS_LANGUAGE][7]%></strong></td>  
                                      ...
                                      %>
                                      <tr class="listgensell">
                                        <td align="center"><%=accPayable.getPaymentNumber()%></td>
                                        <td>
                                        ...
                                        <td>
                                          <%=ControlCombo.draw(FrmAccPayableDetail.fieldNames[FrmAccPayableDetail.FRM_PAYMENT_SYSTEM_ID], null, "" + paymentSystem.getOID(), listPaySysVal, listPaySysKey, "onchange=\"changePaymentSystem('" + oidContact + "');\"", "formElemen")%>
                                      ...
                                      <tr class="listgensell">
                                        <td align="center"><%=accPayable.getPaymentNumber()%></td>    
                                        <td>
                                          <%if (accPayable.getStatus() == 7) { %>
                                          <%=Formater.formatDate(accPayable.getPaymentDate(), "dd-MM-yyyy")%> 
                                          <%if (privUnlock) {%>
                                          <a href="javascript:cmdOpenPayment('<%=oidPayment%>', '<%=oidPaymentDetail%>')">unclock</a>
                                          <%}%>
                                          <%} else {%>
                                          <a href="javascript:cmdEditPayment('<%=oidPayment%>', '<%=oidPaymentDetail%>')"><%=Formater.formatDate(accPayable.getPaymentDate(), "dd-MM-yyyy")%></a>
                                          <%}%>
                                        </td>
                                        ...
                                        <td align="right"><%=(I_DocStatus.fieldDocumentStatus[accPayable.getStatus()])%></td>
                                      </tr>
                                      <%
                                          }
                                        } 
                                      ...
                                      %>
                                      <tr  class="listgensell"> 
                                        <td colspan="8" class="comment" align="center"><%=textGlobalTitle[SESS_LANGUAGE][13]%></td>
                                      </tr>
                                      <% }%>
                                      ...
                                      <tr class="listgensell">
                                        ...
                                        <td><%=ControlCombo.draw(FrmAccPayableDetail.fieldNames[FrmAccPayableDetail.FRM_PAYMENT_SYSTEM_ID], null, "", listPaySysVal, listPaySysKey, "onchange=\"changePaymentSystem('" + oidContact + "');\"", "formElemen")%></td>
                                      ...
                                      <tr id="paymentSystem" style="display: none;"  class="listgensell"> 
                                        ...
                                        <td colspan="5"  class="listgensell"> 
                                          <table>
                                            ...
                                            <tr id="bankAddress" style="display: none;"> 
                                            ...
                                            <tr id="swiftCode" style="display: none;"> 
                                            ...
                                            <tr id="accountName" style="display: none;"> 
                                            ...
                                            <tr id="accountNumber" style="display: none;"> 
                                            ...
                                            <tr id="nameOnCard" style="display: none;"> 
                                            ...
                                            <tr id="cardId" style="display: none;"> 
                                            ...
                                            <tr id="expiredDate" style="display: none;"> 
                                            ...
                                            <tr id="payAddress" style="display: none;"> 
                                            ...
                                            <tr id="bgCheckNumber" style="display: none;"> 
                                            ...
                                            <tr id="dueDate" style="display: none;"> 
                                      ...
                                      <tr  class="listgensell"> 
                                        <td colspan="6" align="right"><strong><%=textGlobalTitle[SESS_LANGUAGE][8]%></strong></td>
                                        <td width="19%" align="right"><strong><%=FRMHandler.userFormatStringDecimal(apPayment)%></strong></td>
                                        <td>&nbsp;</td>
                                      </tr>
                                      <tr class="listgensell"> 
                                        <td colspan="6" align="right"><strong><%=textGlobalTitle[SESS_LANGUAGE][9]%></strong></td>
                            ...
                            <td nowrap align="left" class="command">
                              <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                  <td width="1"><img src="<%=approot%>/images/spacer.gif" width="1" height="1"> </td>
                                    <% if (oidReceive != 0) {%>
                                  <td height="22" valign="middle" colspan="3"><a class="btn-primary btn-lg command" href="javascript:cmdBackToReceive('<%=oidReceive%>','<%=oidPurchaseOrder%>')"><i class="fa fa-backward"> &nbsp;<%=textGlobalTitle[SESS_LANGUAGE][20]%></i></a></td>
                                  <%} else {%>
                                  <td height="22" valign="middle" colspan="3"><a class="btn-primary btn-lg command" href="javascript:cmdBackToSearch()"><i class="fa fa-backward"> &nbsp;<%=textGlobalTitle[SESS_LANGUAGE][3]%></i></a></td>
                                  <%}%>
                                  <% //if(records.size() > 0) { %> 
