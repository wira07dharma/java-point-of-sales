...
<% 
  ...
  if(useBrowse == 0 && !(getlist==1 || getlist==2)) {
	...
	response.sendRedirect("../warehouse/material/dispatch/df_stock_wh_material_list.jsp");
    }
    ...
    if (status == 1) {
        try {
		    ...
		    long locToOid = FRMQueryString.requestLong(request, "loc_to_oid_"+i+"");
                    Date dateTransfer = ControlDate.getDateTime("transfer_date_"+i+"",request);

                    System.out.println("description >> : "+description);
                    // System.out.println("suppNomor >> : "+suppNomor);
                    //System.out.println("locOid >> : "+locOid);
		    System.out.println("locToOid >> : "+locToOid);
                    System.out.println("dateTransfer >> : "+dateTransfer);
                    
                    Vector vectTransfer = (Vector)list.get(i);
                    MatDispatch matdispatch = (MatDispatch)vectTransfer.get(0);
                    //matdispatch.setLocationId(locOid);
		    matdispatch.setDispatchTo(locToOid);
                    matdispatch.setRemark(description);
                    matdispatch.setDispatchDate(dateTransfer);
                    
                    vectTransfer.setElementAt(matdispatch, 0);
                    list.setElementAt(vectTransfer, i);
                }
            }
            listResult = SessMatDispatch.approveDispatchProcess(list);

            if(listResult.size()>0){
		try{
		    session.putValue("BARCODE_DISPATCH", listResult);
		    }catch(Exception e){}
                //response.sendRedirect("../warehouse/material/receive/receive_material_list.jsp");
            }else{
		try{
		    session.removeValue("BARCODE_DISPATCH");
		    }catch(Exception e){}
		}
        } catch (Exception ev) {
        } 
...
<script type="text/javascript">
    function cmdback() {
        document.frmimportopname.action = "importtextfiletransfersrc.jsp";
        document.frmimportopname.submit();
    }

    function cmdapprove(status) {
        document.frmimportopname.action = "importtextfiletransferprocess.jsp";
      ...
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td><!-- #BeginEditable "content" -->
                <%
                    out.println("<form name=\"frmimportopname\" method=\"post\" action=\"importtransfersave.jsp\">");
                %>
		...
		<input type="hidden" name="get_list" value="">
			 ...
			 Vector listFailed = new Vector();
				  ...
				  while (enu.hasMoreElements()) {
				      ...
				      MatDispatchItem matDispatchItem = null;
					  ...
					  if (material.getOID() == 0) {
					      ...
					      matDispatchItem.setMaterialId(material.getOID());
					      matDispatchItem.setQty(result.getQty());
					    ...
					    currencyId = material.getDefaultCostCurrencyId();
					      ...
					      matDispatchItem.setHpp(pricereal);
					      matDispatchItem.setUnitId(unitOid);
					      matDispatchItem.setHppTotal(pricereal * matDispatchItem.getQty());
					      matDispatchItem.setResidueQty(matDispatchItem.getQty());

					      itmList.add(matDispatchItem);
					  }

					  if (location.getOID() == 0) {
					      location.setName("Location is Failed");
					  }
					  if (k == 0) {
					      //out.println("<tr>");
						...
						out.println("<tr>");
						out.println("<td width=\"20%\"><div align=\"left\"><b>From :</b></div></td>");
						out.println("<td width=\"20%\"><div align=\"left\">: "+location.getName()+"</div></td>");
						...
						out.println("<tr>");
						out.println("<td width=\"20%\"><div align=\"left\"><b>To</b></div></td>");
						out.println("<td width=\"20%\"><div align=\"left\">: ");
						Vector vloc = PstLocation.list(0,0,"", "");
						    out.println("<select name=\"loc_to_oid_"+jindx+"\" class=\"formElemen\">");
						    for(int j=0;j<vloc.size();j++){
							Location locationx = (Location)vloc.get(j);
							if(locationx.getOID() != location.getOID()){
							    out.println("<option value="+locationx.getOID()+">"+locationx.getName()+"</option>");
							    }
						    }
						    out.println("</select>");
						out.println("</div></td>");
						...
						out.println("<td width=\"20%\"><div align=\"left\">: "+ControlDate.drawDateWithStyle("transfer_date_"+jindx+"",new Date(), 1, -5, "formElemen", "")+"</div></td>");
						out.println("</tr>");

						out.println("<tr>");
						out.println("<td width=\"20%\"><div align=\"left\"><b>Time</b></div></td>");
						out.println("<td width=\"20%\"><div align=\"left\">: "+ControlDate.drawTimeSec("transfer_date_"+jindx+"", new Date(), "formElemen")+"</div></td>");
						out.println("</tr>");
					  ...
					  if (k == 0) {                                          
					      matDispatch.setLocationId(location.getOID());
					      matDispatch.setDispatchDate(new Date());
					      matDispatch.setDispatchTo(0);
					      matDispatch.setRemark(""+Formater.formatDate(result.getDatetime(), "dd/MM/yyyy HH:mm"));
