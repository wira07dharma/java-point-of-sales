...
<script type="text/javascript">
    ...
    function cmdapprove(status) {
        ...
        document.frmimportopname.action = "importtextfileprocess_1.jsp";
				  ...
				  while (enu.hasMoreElements()) {
					  ...
					  try {
					      ...
					      //material = PstMaterial.fetchBySkuBarcode(result.getProdBarcode());
                                              ...
                                              //Used Vector to get 1 row or 2 row
                                              //new get material based on barcode
                                              Vector listMaterial = PstMaterial.listFetchBarcodeAndSellPrice(0, 0, "", result.getProdBarcode());

                                                 String barcode = result.getProdBarcode();
                                                 String barcodePertama = "";
                                                        barcodePertama = barcode.substring(0, 1);

                                              if(listMaterial.size() == 0 && barcodePertama.equals("0")){
                                                int jmlBarcode = barcode.length();
                                                String newBarcode = "";
                                                newBarcode = barcode.substring(1, jmlBarcode);
                                                listMaterial = PstMaterial.listFetchBarcodeAndSellPrice(0, 0, "", newBarcode);
                                              }
					      ...
					      //location = PstLocation.fetchByCode(result.getLocCode());
                                              ...
                                              if(listMaterial != null && listMaterial.size()>0 ){
                                                 for(int i=0; i<listMaterial.size(); i++) {
                                                     Vector temp = (Vector)listMaterial.get(i);
                                                       Material materialList = (Material)temp.get(0);

                                                       matStockOpnameItem.setMaterialId(materialList.getOID());
                                                       matStockOpnameItem.setQtyOpname(result.getQty());
                                                       matStockOpnameItem.setUnitId(materialList.getDefaultStockUnitId());
                                                       matStockOpnameItem.setCost(materialList.getAveragePrice());
                                                       itmList.add(matStockOpnameItem);
                                                        ...
                                                        out.println("<td>" + materialList.getSku() + "</td>");
                                                        out.println("<td>" + materialList.getBarCode() + "</td>");
                                                        out.println("<td><input type=\"hidden\" name=\"name" + materialList.getOID() + "" + location.getOID() + "" + Formater.formatDate(result.getDatetime(), "yyyyMMdd") + "\" value=\"" + materialList.getOID() + "\">" + materialList.getName() + "</td>");
