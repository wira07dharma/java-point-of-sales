...
public class PstDispatchStockCode extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc, I_Language {
    ...
    public static final int FLD_DISPATCH_MATERIAL_ID = 2;
    ...
    public static DispatchStockCode fetchExc(long oid) throws DBException {
        try {
            ...
            //dyas 20131202
            dispatchStockCode.setDispatchMaterialId(pstDispatchStockCode.getlong(FLD_DISPATCH_MATERIAL_ID));
            dispatchStockCode.setStockCode(pstDispatchStockCode.getString(FLD_STOCK_CODE));
            dispatchStockCode.setStockValue(pstDispatchStockCode.getdouble(FLD_STOCK_VALUE));
    ...
    public static long insertExc(DispatchStockCode materialStockCode) throws DBException {
        try {
            ...
            //dyas 20131202
            pstDispatchStockCode.setLong(FLD_DISPATCH_MATERIAL_ID, materialStockCode.getDispatchMaterialId());
            pstDispatchStockCode.setString(FLD_STOCK_CODE, materialStockCode.getStockCode());
            pstDispatchStockCode.setDouble(FLD_STOCK_VALUE, materialStockCode.getStockValue());
            pstDispatchStockCode.insert();
			//kebutuhan untuk service transfer katalog
			PstDataCustom.insertDataForSyncAllLocation(pstDispatchStockCode.getInsertSQL());
    ...
    public static long updateExc(DispatchStockCode materialStockCode) throws DBException {
        try {
            if (materialStockCode.getOID() != 0) {
				...
				//kebutuhan untuk service transfer katalog
				PstDataCustom.insertDataForSyncAllLocation(pstDispatchStockCode.getUpdateSQL());
                return materialStockCode.getOID();
    ...
    public static long deleteExc(long oid) throws DBException {
        try {
			...
			//kebutuhan untuk service transfer katalog
			PstDataCustom.insertDataForSyncAllLocation(pstDispatchStockCode.getDeleteSQL());
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
            throw new DBException(new PstDispatchStockCode(0), DBException.UNKNOWN);
    ...
    private static void resultToObject(ResultSet rs, DispatchStockCode materialStockCode) {
        try {
            ...
            //dyas 20131202
            materialStockCode.setDispatchMaterialId(rs.getLong(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_DISPATCH_MATERIAL_ID]));
            materialStockCode.setStockCode(rs.getString(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_STOCK_CODE]));
            materialStockCode.setStockValue(rs.getDouble(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_STOCK_VALUE]));
        } catch (Exception e) {
        }
    }
    ...
    public static Vector listDispatchStockCodePosting(int limitStart, int recordToGet, String whereClause, String order) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        try {
            /**
             * SELECT PRMIC.*,PRMI.MATERIAL_ID FROM pos_df_material_item_code AS PRMIC
                INNER JOIN pos_dispatch_material_item AS PRMI ON PRMI.DISPATCH_MATERIAL_ITEM_ID=PRMIC.DISPATCH_MATERIAL_ITEM_ID
                INNER JOIN pos_dispatch_receive_item AS PRM ON PRM.DISPATCH_MATERIAL_ID=PRMI.DISPATCH_MATERIAL_ID
                WHERE PRM.DISPATCH_MATERIAL_ITEM_ID=504404544238714021;
             */
            String sql = " SELECT PRMIC.*,PRMI.MATERIAL_ID FROM " + TBL_POS_DISPATCH_MATERIAL_CODE+" AS PRMIC "+
                         " INNER JOIN pos_dispatch_material_item AS PRMI ON PRMI.DISPATCH_MATERIAL_ITEM_ID=PRMIC.DISPATCH_MATERIAL_ITEM_ID "+
                         " INNER JOIN pos_dispatch_material AS PRM ON PRM.DISPATCH_MATERIAL_ID=PRMI.DISPATCH_MATERIAL_ID ";
    ...
    private static void resultToObjectToPosting(ResultSet rs, SourceStockCode materialStockCode) {
        try {
            materialStockCode.setOID(rs.getLong(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_DISPATCH_MATERIAL_CODE_ID]));
            materialStockCode.setSourceId(rs.getLong(PstMatDispatchItem.fieldNames[PstMatDispatchItem.FLD_MATERIAL_ID]));
            materialStockCode.setStockCode(rs.getString(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_STOCK_CODE]));
            materialStockCode.setStockValue(rs.getDouble(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_STOCK_VALUE]));
        } catch (Exception e) {
        }
    ...
    public static Vector checkListStockCodeDispatch(long oidDocItem) {
        Vector lists = new Vector();
        DBResultSet dbrs = null;
        boolean result = false;
        try {
            String sql = "SELECT " +
                            ...
                            "," + PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_DISPATCH_MATERIAL_ID] +
            ...
            while (rs.next()) {
                  DispatchStockCode materialStockCode = new DispatchStockCode();//(SourceStockCode)
                  resultToObject(rs, materialStockCode);
                  lists.add(materialStockCode);
                //result = true;
            }
    ...
    synchronized public static boolean automaticInsertSerialNumber(double cnt, long oid,double strVal, long locationId, long dispatcMaterialItemId){
          boolean insert = true;
          try{
              ...
              if (list != null && list.size() > 0) {
                   for(int k=0;k<list.size();k++){      
                        ...
                        DispatchStockCode dispatchStockCode = new DispatchStockCode();
                        dispatchStockCode.setDispatchMaterialItemId(dispatcMaterialItemId);
                        dispatchStockCode.setStockCode(""+materialStockCode.getStockCode());
                        dispatchStockCode.setStockValue(strVal);
                        //dyas 20131130
                        //tambah setReceiveMaterialId untuk mengambil nilai yang dibawa oleh oidReceive
                        dispatchStockCode.setDispatchMaterialId(oid);
                        try{
   ...
   public static JSONObject fetchJSON(long oid){
      ...
      try {
         DispatchStockCode dispatchStockCode = PstDispatchStockCode.fetchExc(oid);
         object.put(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_DISPATCH_MATERIAL_CODE_ID], dispatchStockCode.getOID());
         object.put(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_DISPATCH_MATERIAL_ITEM_ID], dispatchStockCode.getDispatchMaterialItemId());
         object.put(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_DISPATCH_MATERIAL_ID], dispatchStockCode.getDispatchMaterialId());
         object.put(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_STOCK_CODE], dispatchStockCode.getStockCode());
         object.put(PstDispatchStockCode.fieldNames[PstDispatchStockCode.FLD_STOCK_VALUE], dispatchStockCode.getStockValue());
      }catch(Exception exc){}
      return object;
   }
}
