...
public class PstArApPayment extends DBHandler implements I_DBInterface, I_DBType, I_PersintentExc {
	...
	return updateExc(arap, null);
    }
    
    public static long updateExc(ArApPayment arap, Connection objConnection) throws DBException {
        try {
		...
		if(objConnection == null){
		    ...
		    pstArApPayment.updateTran(objConnection);
		}
                return arap.getOID();
            }
        } catch (DBException dbe) {
    ...
    public static long deleteExc(long Oid, Connection objConnection) throws DBException {
		...
		pstArApPayment.deleteTran(objConnection);  
	    }
        } catch (DBException dbe) {
            throw dbe;
        } catch (Exception e) {
    ...
    public static int getCount(String whereClause) {
        DBResultSet dbrs = null;
        try {
            String sql = "SELECT DISTINCT "+ PstArApPayment.fieldNames[PstArApPayment.FLD_COUNTER] +
                    " FROM " + TBL_ARAP_PAYMENT;
            if (whereClause != null && whereClause.length() > 0){
                sql +=  " WHERE " + whereClause+
			" ORDER BY "+PstArApPayment.fieldNames[PstArApPayment.FLD_COUNTER]+" DESC ";
	    }else{
		 sql += " ORDER BY "+PstArApPayment.fieldNames[PstArApPayment.FLD_COUNTER]+" DESC ";
	    }

	    System.out.println("SQL PstArApPayment.getCount() :::::::::::: "+sql);
            dbrs = DBHandler.execQueryResult(sql);
            ResultSet rs = dbrs.getResultSet();
            ...
            while (rs.next()) {
		if(rs.isFirst()){
		    count = rs.getInt(PstArApPayment.fieldNames[PstArApPayment.FLD_COUNTER]);
		}
            }
            rs.close();
            return count;
    ...
    public static ArApPayment createOrderNomor(ArApPayment arApMain) {
        String nmr = "";
	//System.out.println("DBSVR_TYPE ::::::::::::::::::: "+DBSVR_TYPE);
        try {
