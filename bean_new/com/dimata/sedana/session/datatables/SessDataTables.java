...
package com.dimata.sedana.session.datatables;
...
public class SessDataTables {
  ...
  private String generateQueryAll() {
    ...
    this.query += (sCol < 0) ? "" : " ORDER BY " + this.getAlias(x.getColName()) + this.dir;
    ...
    try {
      ...
      while (rs.next()) {
        ...
        for(Column c: this.cols) {
          String p = "";
          if(c.isCustom()) {
            p = c.customAction(rs.getString(col), d);
          } else if(c.isAction()) {
  ...
  public class Column {
    protected String customAction(String rsValue, HashMap<Integer, String> data) {
      return rsValue;
    }
